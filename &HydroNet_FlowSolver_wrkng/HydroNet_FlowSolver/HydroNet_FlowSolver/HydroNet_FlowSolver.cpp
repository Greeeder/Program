...
void HydroNet_FlowSolver(std::vector<strBranches> branches, std::vector<strPump_volum> pump_volum, std::vector<std::vector<int>>branches_id, std::vector<double> bound_flows, std::vector<int> nodes_id, std::vector<std::vector<int>> mesh_branches, std::vector<std::vector<int>> node_branches, int n_mesh, int n_branch, std::vector<double> head_loss, std::vector<double> hydr_resist1, std::vector<double> hydr_resist2, int n_tanks) {
	...
	double sol_aux;
	int n_unknown;
	std::vector<double> coef1_mat, coef2_mat, const_mat, const_vec;
	int unk_ind_aux;
	int volpump_ind_aux, flows_ind_aux;
	int last_row;
	double n_idp_mesh;
	std::vector<double> idp_mesh;
...
// 1. INDEPENDENT FLOWS
	...
	for (int count; count < n_branch; count++)
	...
	for (int count = 1; count < branches.size(); count++)																// Enter every branch
		for (int brnch_aux = 1, aux = 0, branch_aux; branch_aux < branches.at(count).objects.size(); branch_aux++)		 //Looks objects in branch
	//	branch_aux = objects.branch{ pump_volum.obj_index(count) };													// pump branch
			if (solved.at(count) = false && branches.at(count).objects.at(branch_aux).Class == "pump_volum") {		//Enters when the object is a pump_volum										// the branch is not solved yet
				volum_pump.at(count) = true;																		// there is a volumetric pump
				while (branches.at(count).objects.at(branch_aux).ID != pump_volum.at(aux).Pump_volum)				// Looks up for the pump at the branch in the pump_volum vector
					aux++;
				//calculates flow

				flows.at(count) = pump_volum.at(aux).coef0 + pump_volum.at(aux).coef1 * pump_volum.at(aux).pump_speed + pump_volum.at(aux).coef2 * pump_volum.at(aux).pump_speed * pump_volum.at(aux).pump_speed + pump_volum.at(aux).coef3 * pump_volum.at(aux).pump_speed * pump_volum.at(aux).pump_speed * pump_volum.at(aux).pump_speed;
				solved.at(count) = true; // set as solved
			}
...
// 2. LOOP FOR NODES WITH ONE UNKNOWN FLOW
	...
	while ((solved != solv_prev1)) {												// in the previous step, some flow was solved(NODES + MESHES)
		...
		while (solved != solv_prev2) {												// in the previous step, some flow was solved(NODES)
			...
			for (int count = 1; count < n_nodes; count++) {
				count_uns = 0;																	 // counter of unsolved branches
				for (int count1 = 1; count1 < node_branches.at(count).size(); count1++) {			// branches connected to node
				...
				if (count_uns == 0)													// all connected branches are solved
					...
					for (int count1; count < node_branches.at(count).size(); count++) {
						// branches connected to node
						// Node can be at the end or at the begining of the branch
						// Criterium: positive flow--> enters node
						if (nodes_id.at(count) == branches_id.at(count1).at(branches_id.at(count1).size() - 1))			// at the end, keep sign
																												// flow enters the node
							flow_sum = flow_sum + flows.at(count1);
						else if (nodes_id.at(count) == branches_id.at(count1).at(1))								// at the beginning, change sign
																											// flow leaves the node
							flow_sum = flow_sum - flows.at(count1);												// change sign
					}
					...
					ind_aux = node_branches.at(count).at(ind_aux);									// change ind_aux to refer directly to the branch or flow index
																									...
																									// Node can be at the end or at the begining of the branch
					if (nodes_id.at(count) == branches_id.at(ind_aux).at(1))									// at the beginning
		...
		while (solved != solv_prev2) {																		// in the previous step, some flow was solved(MESH)
			...
			for (int count = 1; count < n_mesh; count++) {														 // meshes
				count_uns = 0;																				// counter of unsolved branches
				for (int count1 = 0; count1 < mesh_branches.at(count); count1++) {										 // branches that form the mesh
				...
				if (count_uns == 1) {																			 // one unknown; mesh can be solved directly
					...
					total_head = 0;
					last_id = 0;
					for (int count1 = 0; count1 < mesh_branches.at(count); count++) {								// branches that form the mesh
						...
						if (count1 != ind_aux || volum_pump.at(count1) == true) {									// flow in the branch is known
							...
							// Add head increment
							total_head = total_head + (head_loss.at(count1) + hydr_resist1.at(count1) * fabs(flows.at(count1)) + hydr_resist2.at(count1) * flows.at(count1) ^ 2)* (-2 * change_sign + 1);
						...
						if (volum_pump.at(ind_aux) == true) {																	// the unknown branch has a volumetric pump
							...
							// Solve branch
							_sol_aux = roots([hydr_resist2.at(ind_aux) hydr_resist1.at(ind_aux) (head_loss(ind_aux) - abs(total_head))]);
...
// flows - solved flows + volumetric pump head
	...
	coef2_mat.assign(n_unknown,0); // coefficients for unknowns ^ 2 in each equation(row)
	coef1_mat.assign(n_unknown, 0); // coefficients that multiply unknowns in each equation(row)
	const_mat.assign(n_unknown, 0); // matrix for independent head loss in unsolved branches
...
// independent head loss and head calculated with known flows
	...
	for (int count = 1; count < sum(not(solv_nodes)); count++) { // unsolved nodes loop

		// Continuity equation : sum of flows in a node = 0
		for (int count1 = 0; count1 < node_branches.at(count).size ; count1++) {// branches connected to node(branch index)

			// Flow may be solved or not
			if (solved.at(count1) == true) // the flow in this branch has been solved
				const_vec.at(count) = flows.at(count1);
	...
	if (n_mesh > n_idp_mesh) { // there are more meshes than necessary->selection
		for (int count = 1; count < n_idp_mesh; count++) { // selected mesh loop
			...
			for (int count1 = 1; count1 < n_mesh; count++) { // mesh loop
				count_uns = 0; // counter of unsolved branches included in the selected mesh
				for (int count2 = 0; count2 < transpose(mesh_branches.at(count1); count2++) { // branches contained in the mesh(vector index)
					if (remain_branch.at(count2) == true) // branch must be solved
			...
			// Remove all branches contained in the selected mesh from the remaining branches
				for (int count2 = 0; count2 < transpose(mesh_branches.at(best_mesh); count2++) { // branches contained into mesh(vector index)
					remain_branch.at(count2) = false; // branch does not have to be solved
				}
		}
	}
	...
	idp_mesh_branches = mesh_branches.at(idp_mesh);


	// Initial values for the non - linear solver
	x0.assign(n_unknown, 0.001) ; // size equal to number of unknowns
	...
	for (int count = 1; count < n_idp_mesh; count++) { // independent mesh loop

		// II Kirchhoff's Law: sum of head increments in a mesh = 0
		last_id = 0;
		for (int count1 = 0; count1< transpose(idp_mesh_branches.at( count ));count1++) {// branches in the mesh(branches index)
			// Branches are already ordered along the mesh
			// Criterium: positive mesh flow-- > direction of the first branch in the mesh
			// If there is a pump or a heat exchanger of type 'T out' in the branch, flow and branch direction are the same
			// If flow direction is unknown, assume same direction as branch
			...
			if (last_id == 0 || branches_id.at(count1).at(1) == last_id)
				// it is the first branch in the mesh OR the branch is in the same direction as the mesh flow
				last_id = branches_id.at(count1).at(branches_id.at(count1).size-1); // refresh other end of branch
			else { // it is not the first branch in the mesh AND the branch is in the opposite direction
				change_sign = true; // invert sign
				last_id = branches_id.at(count1).at(1); // refresh other end of branch
			}
			...
			if (solved.at(count1)==true) {// the flow in this branch has been solved
				...
				const_vec.at(last_row + count) = const_vec.at(last_row + count) + (head_loss.at(count1) + hydr_resist1.at(count1) * abs(flows.at(count1)) + hydr_resist2.at(count1) * flows.at(count1) ^ 2) * (-2 * change_sign + 1);
				// add head if change_sign = false; subtract if change_sign = true
				if (volum_pump.at(count1)) // if there is a volumetric pump, include pump head as an unknown variable of the system
					coef1_mat.at(last_row + count, (unk_ind_aux == count1)) = -(-2 * change_sign + 1) * 10000; // include pump head
				...
				const_mat.at(last_row + count, (unk_ind_aux == count1)) = head_loss.at(count1) * (-2 * change_sign + 1);
				coef1_mat.at(last_row + count, (unk_ind_aux == count1)) = hydr_resist1.at(count1) * (-2 * change_sign + 1);
				coef2_mat.at(last_row + count, (unk_ind_aux == count1)) = hydr_resist2.at(count1) * (-2 * change_sign + 1);
			}
		}
	}
