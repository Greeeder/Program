...
void HeatExchanger_model::create_HeatExchangers()
{
	...
	while (!HeatExchanger_file.eof()) {
		HeatExchanger_file >> efficiency_data >> "\t";	// Save in efficiency_data
		line++;	// lines counter
	}
	HeatExchanger_file.close();

	// Array data is converted and stored
	HeatExchangers.at(count).NTU.resize(line);
	HeatExchangers.at(count).Cr.resize(line);
	HeatExchangers.at(count).efficiency.resize(line);

	std::string delimiter = "\t";
	size_t pos = 0;
	line = 0;
	while ((pos = efficiency_data.find(delimiter)) != std::string::npos) {
		HeatExchangers.at(count).NTU[line] = std::stod(efficiency_data.substr(0, pos));
		efficiency_data.erase(0, pos + delimiter.length());
		HeatExchangers.at(count).Cr[line] = std::stod(efficiency_data.substr(0, pos));
		efficiency_data.erase(0, pos + delimiter.length());
		HeatExchangers.at(count).efficiency[line] = std::stod(efficiency_data.substr(0, pos));
		efficiency_data.erase(0, pos + delimiter.length());
		line++;
	}
	...
	results.at(count).HeatExchanger_ID = HeatExchanger.at(count).ID_htx;
