...
void HeatExchanger_model::create_HeatExchangers(int n_htxs)
{
	...
	// Read lines of data																			//HeatExchanger_file.open("HeatExchanger_inputs.txt", std::ifstream::out );
																				//OPEN = HeatExchanger_file.is_open();
		...
		while (HeatExchanger_file) {
			std::getline(HeatExchanger_file, data);	// Save line in HeatExchanger_data

			HeatExchanger_data.push_back(data);
		...
		if (HeatExchangers.at(count).flag_efficiency_table == true) {
			...
			while (HeatExchanger_file) {
				...
				NTU_data.push_back(data);

				std::getline(HeatExchanger_file, data, '\t');
				
				Cr_data.push_back(data);

				std::getline(HeatExchanger_file, data, '\n');

				efficiency_data.push_back(data);
...
void HeatExchanger_model::calculate_heat_exchange()
{
	...
	mCp_1 = inputs.at(count).mass_flow_1 * cp(HeatExchangers.at(ID_htxch).fluid_type_1, inputs.at(count).Temperature_inlet_1);
	mCp_2 = inputs.at(count).mass_flow_2 * cp(HeatExchangers.at(ID_htxch).fluid_type_2, inputs.at(count).Temperature_inlet_2);
	mCp_min = std::min(mCp_1, mCp_2);
	mCp_max = std::max(mCp_1, mCp_2);

	NTU = HeatExchangers.at(ID_htxch).UA / mCp_min;
	Cr = mCp_min / mCp_max;

	if (HeatExchangers.at(ID_htxch).flag_efficiency_table) {	// look up efficiency in a table
		efficiency = lookup_table_2dim(HeatExchangers.at(ID_htxch).n_rows_table, NTU, Cr, &HeatExchangers.at(ID_htxch).NTU, &HeatExchangers.at(ID_htxch).Cr, &HeatExchangers.at(ID_htxch).efficiency);
	}
	else {	// no table; obtain efficiency analiticaly considering the heat exchanger characteristics
		switch (HeatExchangers.at(ID_htxch).exchanger_type) {
		...
		case 3:	// Shell and tube
			...
			if (HeatExchangers.at(ID_htxch).n_shell_passes > 1) {	// multiple passes through shell (for 1, the previous expression is obtained)
				efficiency = (pow(((1 - efficiency * Cr) / (1 - efficiency)), HeatExchangers.at(ID_htxch).n_shell_passes) - 1)
					/ (pow(((1 - efficiency * Cr) / (1 - efficiency)), HeatExchangers.at(ID_htxch).n_shell_passes) - Cr);
			}
			break;
