...
void TransmisionCalor::crear_nodos_fluidos()
{
	...
	// 1 not used (to avoid starting at 0), chamber gas, intake, gas, exhaust gas, oil, liner coolant, cylinder-head coolant (7 so far),
	// n_axials - 1 of chamber gas in temporary contact with the liner surface (top liner node is always in contact)
...
void TransmisionCalor::crear_nodos_base()
{
	...
	// Engine geometry and liner conductivity
	dp = _Inicializar.ConfMotor_BASE_D;
	s = _Inicializar.ConfMotor_BASE_S;
	h0 = _Inicializar.ConfMotor_PISTON_HLI;
	espesor = _Inicializar.Constante_TRANS_CALOR_C_ECIL * dp; // liner thickness
	esp_agua = espesor / 2; // thickness coolant jacket
	...
	ncilin = naxiales * nradiales * ncircunf + ncircunf; // number of nodes in the liner
	_NODOS_CIL.resize(ncilin + 1);	// +1: to start rows from 1 instead of from 0 (first line is ignored)
	

	// Lengths of axial nodes
	...
	// Opening angle of circumferential nodes
	// First circumferential node is not cooled and its size is user defined
	ang_no_cool = _Inicializar.ConfMotor_BASE_ANG_NO_REFR_CIL;
	if (ang_no_cool == 0) { // to avoid dividing by zero
		ang_no_cool = 0.1;
	}
	// Opening angle of all other circumferential nodes is the same
	delta_fi = (360.0 - ang_no_cool) / (ncircunf - 1);
	// Los nodos circunferenciales son todos del mismo tamaño:
	//delta_fi = 360. / ncircunf;


	// Liner nodes loop
	fila = 2; // Initialize _NODOS_CIL row
	numero = 1;	// Initialize liner node number
	for (cont1 = 1; cont1 <= naxiales; cont1++) {
		for (cont2 = 1; cont2 <= nradiales; cont2++) {
			for (cont3 = 1; cont3 <= ncircunf; cont3++) {
				_pos_matriz++; // initialized in "crear_nodos_fluidos"
				...
				if (cont1 == 1) { // first axial
					_NODOS_CIL.at(fila - 1).L_SUP = Lcil;
				}
				else {
					Lapoyo = 0; // auxiliary length
				...
				if (cont3 == 1) { // first circumferential
					_NODOS_CIL.at(fila - 1).ANG_INI = 0.0;
				}
				else {
					_NODOS_CIL.at(fila - 1).ANG_INI = ang_no_cool + delta_fi * (cont3 - 2);
				}
				_NODOS_CIL.at(fila - 1).ANG_FIN = ang_no_cool + delta_fi * (cont3 - 1);
				//_NODOS_CIL.at(fila - 1).ANG_INI = delta_fi * (cont3 - 1);
				//_NODOS_CIL.at(fila - 1).ANG_FIN = delta_fi * (cont3);
				_NODOS_CIL.at(fila - 1).volume = Constantes::pi / 4.0 * (_NODOS_CIL.at(fila - 1).D_MAX * _NODOS_CIL.at(fila - 1).D_MAX
					- _NODOS_CIL.at(fila - 1).D_MIN * _NODOS_CIL.at(fila - 1).D_MIN) * (_NODOS_CIL.at(fila - 1).ANG_FIN
					- _NODOS_CIL.at(fila - 1).ANG_INI) / 360.0 * abs(_NODOS_CIL.at(fila - 1).L_SUP - _NODOS_CIL.at(fila - 1).L_INF);
				_NODOS_CIL.at(fila - 1).k = k_cil;
				numero = numero + 1;
				fila = fila + 1; // next row or node
			}
		}
	}


	// "Cantilever" nodes over the coolant jacket (axial pos = 1, radial pos = nradial + 1)
	//Nodo del cilindro  justo encima del refrigerante y de longitud h0
	for (cont1 = (1 + naxiales * nradiales * ncircunf); cont1 <= ncilin; cont1++) { // continue nodes (this counter was formerly represented by counter "fila")
		// Node at the same axial and circumferential position but radially one position closer to chamber, to use as reference
		cont2 = cont1 - naxiales * nradiales * ncircunf + (nradiales - 1) * (ncircunf); //identifica al nodo del cilindro de mayor radio y posicion superior
		_pos_matriz++;
		_NODOS_CIL.at(cont1).ID_NODO = _pos_matriz;
		_NODOS_CIL.at(cont1).NOMBRE = "CIL" + std::to_string(_NODOS_CIL.at(cont2).POS_AX) + std::to_string(nradiales + 1) + std::to_string(_NODOS_CIL.at(cont2).POS_CIRC);
		//_NODOS_CIL.at(cont1).NOMBRE = "CIL" + std::to_string(cont1) + std::to_string(cont2) + std::to_string(cont3); //(JS) invalid expression for this loop 
		...
		_NODOS_CIL.at(cont1).D_MAX = _NODOS_CIL.at(cont2).D_MAX + 2.0 * esp_agua;
		...
		_NODOS_CIL.at(cont1).volume = Constantes::pi / 4 * (_NODOS_CIL.at(cont1).D_MAX * _NODOS_CIL.at(cont1).D_MAX
			- _NODOS_CIL.at(cont1).D_MIN * _NODOS_CIL.at(cont1).D_MIN) * (_NODOS_CIL.at(cont1).ANG_FIN
			- _NODOS_CIL.at(cont1).ANG_INI) / 360.0 * abs(_NODOS_CIL.at(cont1).L_SUP - _NODOS_CIL.at(cont1).L_INF);
		_NODOS_CIL.at(cont1).k = k_cil;
		numero = numero + 1;
	}
...
double TransmisionCalor::resuelve_suma_geometrica(double suma, short n)
{
	...
	//suma=[A^n - 1]/[A-1]
	// suma es la longitud total y n es el número de datos
	///''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	double A0;
	double a1;
	double A2;
...
void TransmisionCalor::crear_nodos_piston()
{
	...
	dp = _Inicializar.ConfMotor_BASE_D; // repeated
	DB = _Inicializar.ConfMotor_PISTON_DB;
	k_pis = _Inicializar.ConfMotor_PISTON_CONDUCTIVIDAD;
	//C_DIAM_INT_GAL = _Inicializar.Constante_TRANS_CALOR_C_DIAM_INT_GAL; (not used anywhere)
	C_DIAM_INT_GAL = _Inicializar.ConfMotor_PISTON_DIAM_INT_GAL; // used in the rest of the script
	Dgal = C_DIAM_INT_GAL * dp;


	//Define geometria de nodos del piston (L exialmente y D radialmente)
	if (DB <= 0)
		DB = 0.5 * dp; //J. Salvador: I changed it because it seemed very high to me
		//DB = 0.9 * dp;
	// Heights from piston bottom
	Lp = f_Lp * dp;
	L1p = f_L1 * dp;
	L2p = f_L2 * dp;
	L3p = f_L3 * dp;
	...
	L5p = f_L5 * dp;
	L6p = 0.5 * (L5p + L4p);
	// Radial distances from dp/2 to vertical axis
	D2p = (dp - DB) / 2.0;
	D4p = dp / 2.0;
	D3p = (D2p + D4p) / 2.0;
	D1p = f_D1 * D3p;
	...
	npis = 10; // number of piston nodes
	_NODOS_PIS.resize(npis + 1);	// +1: to start rows from 1 instead of from 0 (first line is ignored)


	// Piston nodes: identification and corner coordinates
	cont = 1;
	_pos_matriz++;
	_NODOS_PIS.at(cont).ID_NODO = _pos_matriz;
	_NODOS_PIS.at(cont).NOMBRE = "PIS" + std::to_string(cont);
	_NODOS_PIS.at(cont).numero = cont;
	_NODOS_PIS.at(cont).X_II = 0.0;
	_NODOS_PIS.at(cont).Y_II = L4p;
	_NODOS_PIS.at(cont).X_SI = 0.0;
	...
	_NODOS_PIS.at(cont).numero = cont;
	_NODOS_PIS.at(cont).X_II = 0.0;
	_NODOS_PIS.at(cont).Y_II = 0.0;
	_NODOS_PIS.at(cont).X_SI = 0.0;
	_NODOS_PIS.at(cont).Y_SI = L1p;
	_NODOS_PIS.at(cont).X_SD = D1p;
	_NODOS_PIS.at(cont).Y_SD = L1p;
	_NODOS_PIS.at(cont).X_ID = D1p;
	_NODOS_PIS.at(cont).Y_ID = 0.0;
	...
	// Width, height, geometric center, areas and conductivity
	for (cont0 = 1; cont0 <= 10; cont0++) {

		_NODOS_PIS.at(cont0).ANCHURA = _NODOS_PIS.at(cont0).X_SD - _NODOS_PIS.at(cont0).X_II;	// width = X upper right corner - X lower left corner
		_NODOS_PIS.at(cont0).ALTURA = _NODOS_PIS.at(cont0).Y_SI - _NODOS_PIS.at(cont0).Y_II;	// height = Y upper left corner - X lower left corner

		// Geometric center coordinates (for distances)
		if ((cont0 == 6) | (cont0 == 8)) { // triangular shape
			_NODOS_PIS.at(cont0).X_CG = _NODOS_PIS.at(cont0).X_II + _NODOS_PIS.at(cont0).ANCHURA / 3.0;
			_NODOS_PIS.at(cont0).Y_CG = _NODOS_PIS.at(cont0).Y_SI - _NODOS_PIS.at(cont0).ALTURA / 3.0;
		}
		else if ((cont0 == 9) | (cont0 == 10)) { // around the vertical axis
			_NODOS_PIS.at(cont0).X_CG = _NODOS_PIS.at(cont0).X_II + _NODOS_PIS.at(cont0).ANCHURA; // X coordinate of GC is in the vertical axis
			_NODOS_PIS.at(cont0).Y_CG = _NODOS_PIS.at(cont0).Y_II + _NODOS_PIS.at(cont0).ALTURA / 2.0;
		}
		else {
			_NODOS_PIS.at(cont0).X_CG = _NODOS_PIS.at(cont0).X_II + _NODOS_PIS.at(cont0).ANCHURA / 2.0;
			_NODOS_PIS.at(cont0).Y_CG = _NODOS_PIS.at(cont0).Y_II + _NODOS_PIS.at(cont0).ALTURA / 2.0;
		}
		...
		Aux = 0;		
		if (((cont0 == 6) | (cont0 == 8))) {
		Aux = sqrt(_NODOS_PIS.at(cont0).ANCHURA * _NODOS_PIS.at(cont0).ANCHURA + _NODOS_PIS.at(cont0).ALTURA * _NODOS_PIS.at(cont0).ALTURA) - _NODOS_PIS.at(cont0).ALTURA;
...
//			Aux = pot3 - _NODOS_PIS.at(cont0).ALTURA;
		...
		_NODOS_PIS.at(cont0).A_S = Constantes::pi * ((dp / 2.0 - _NODOS_PIS.at(cont0).X_II) * (dp / 2.0 - _NODOS_PIS.at(cont0).X_II) - (dp / 2.0 - _NODOS_PIS.at(cont0).X_SD) * (dp / 2.0 - _NODOS_PIS.at(cont0).X_SD));
		_NODOS_PIS.at(cont0).A_I = Constantes::pi * ((dp / 2.0 - _NODOS_PIS.at(cont0).X_II + Aux) * (dp / 2.0 - _NODOS_PIS.at(cont0).X_II + Aux) - (dp / 2.0 - _NODOS_PIS.at(cont0).X_SD) * (dp / 2.0 - _NODOS_PIS.at(cont0).X_SD));
//		_NODOS_PIS.at(cont0).A_S = Constantes::pi * (pow((dp / 2.0 - _NODOS_PIS.at(cont0).X_II), 2) - pow((dp / 2.0 - _NODOS_PIS.at(cont0).X_SD), 2));
//		_NODOS_PIS.at(cont0).A_I = Constantes::pi * (pow((dp / 2.0 - _NODOS_PIS.at(cont0).X_II + Aux), 2) - pow((dp / 2.0 - _NODOS_PIS.at(cont0).X_SD), 2));
		_NODOS_PIS.at(cont0).A_LI = 2 * Constantes::pi * _NODOS_PIS.at(cont0).ALTURA * (dp / 2 - _NODOS_PIS.at(cont0).X_II);
		if (((cont0 == 6) | (cont0 == 8))) {
			_NODOS_PIS.at(cont0).A_LD = 0.0;
			_NODOS_PIS.at(cont0).Area = _NODOS_PIS.at(cont0).ANCHURA * _NODOS_PIS.at(cont0).ALTURA / 2;
		}
		else if ((cont0 == 9) | (cont0 == 10)) {  //(JS) A_LD should be 0 (even though it is not used)
			_NODOS_PIS.at(cont0).A_LD = 0.0;
			_NODOS_PIS.at(cont0).Area = _NODOS_PIS.at(cont0).ANCHURA * _NODOS_PIS.at(cont0).ALTURA;
		}
		else {
			_NODOS_PIS.at(cont0).A_LD = 2.0 * Constantes::pi * _NODOS_PIS.at(cont0).ALTURA * (dp / 2.0 - _NODOS_PIS.at(cont0).X_SD);
			_NODOS_PIS.at(cont0).Area = _NODOS_PIS.at(cont0).ANCHURA * _NODOS_PIS.at(cont0).ALTURA;
		}


		// Volumes
		if ((cont0 == 6) | (cont0 == 8)) { // triangular
			_NODOS_PIS.at(cont0).volume = 2.0 * Constantes::pi * (dp / 2.0 - _NODOS_PIS.at(cont0).X_CG) * _NODOS_PIS.at(cont0).Area; // 2 * pi * radius * section area
		}
		else {
			_NODOS_PIS.at(cont0).volume = _NODOS_PIS.at(cont0).A_S * _NODOS_PIS.at(cont0).ALTURA;
		}
	...
	// Node 3 contains an oil gallery and thus its volume must be deducted from the solid volume
	_NODOS_PIS.at(3).volume = _NODOS_PIS.at(3).volume - Constantes::pi * Constantes::pi * Dgal * Dgal * 0.5 * (dp / 2.0 - _NODOS_PIS.at(3).X_SD);
	// Torus volume:  1/4 * pi^2 * diameter section circumference ^2 (assumed circular) *
	//	twice the distance from the rotation center to the center of the section circumference (approximated by distance from vertical axis to node 3)
	//Volumen toro: 1/4 * pi^2 * diámetro circunferencia de sección ^2 (supone galería circular) *
	//	2 veces la distancia del centro de rotación al centro de la circunferencia de sección (lo aproxima por la distancia desde el eje vertical hasta el nodo 3)


	// Area correction for nodes exposed to chamber gas	//	[CALMEC]
	//Corregimos el área de los nodos del piston expuestos al gas teniendo en cuenta el área del pistón introducida por el usuario, si no hubiera área del pistón no se hace la corrección
	if (_Inicializar.ConfMotor_PISTON_AP != Constantes::ValorNulo) //(JS) made optional
	{
		double area_nodal;
		double factor_area;
...
void TransmisionCalor::crear_nodos_culata()
{
	...
	double C_ECUL;
	double C_DINY;
	double F_VAST_ADM;	//Proporcion entre el diámetro de la válvula y el diamétro del vástago
	double F_VAST_ESC;
	double k_cul;
	...
	_NODOS_CUL.resize(36); // 35 nodes in the cylinder-head	// +1: to start rows from 1 instead of from 0 (first line is ignored)
	...
	//F_VAST_ADM = 0.2
	//F_VAST_ESC = 0.25
	k_cul = _Inicializar.ConfMotor_CULATA_CONDUCTIVIDAD;


	// Basic parameters
	DINY = dp * C_DINY;	// injector diameter
	espesor_FD = C_ECUL * dp;	// firedeck thickness: two node levels
	espesor_pipas = C_ECIL * dp;	// port wall thickness
	Lcul = dp + 2.0 * espesor;	// cylinder-head width (in both directions)
	L_per = Lcul / 2.0;

	// Valve center position (X coordinate = Y coordinate)
	//Posicion del centro de la válvulas tanto en x como en y
	//c_val = (dp / 2.0 - DINY / 2.0) / 2.0 ^ (0.5) 'El cateto sabiendo la hipotenusa
	c_val = (dp / 4.0 + DINY / 4.0) / (Raiz2);

	// Cylinder-head height
	//Alto de la culata
	if ((dp - c_val + DVA / 2.0 + 2.0 * espesor_pipas / 2.0) > dp) {
		Espesor_zona1_2 = dp - c_val + DVA / 2.0 + 2.0 * espesor_pipas / 2.0;
	}
	else {
		Espesor_zona1_2 = dp; // cylinder-head height excluding firedeck
	}


	// Central node: area and geometric center.  Auxiliary areas and coordinates.
	///Posiciones cdg de valvulas, inyector y zona central
	// Central node //sector de culata cuadrado
	STotal = c_val * c_val;	// area 1/4 central squared node
	//xtotal = c_val / 2.0;	// X coordinate of the GC of that squared area
	Ytotal = c_val / 2.0;	// Y coordinate of the GC of that aquared area
	// Injector //inyector
	Siny = Constantes::pi / 4.0 * DINY * DINY / 4.0; // area 1/4 injector node (circular sector)
	//xiny = 4.0 / 3.0 * (Raiz2) / Constantes::pi  * (DINY / 2.0) * cos(Constantes::pi / 4.0); // X coordinate of the GC of the circular sector
	yiny = 4.0 / 3.0 * (Raiz2) / Constantes::pi  * (DINY / 2.0) * sin(Constantes::pi / 4.0);	// 4.0 / 3.0 -> to be considered as doubles intead of integers and the result to be a double, not only the integer part
	// Exhaust valve //valvula de escape
	SVE = Constantes::pi / 4.0 * DVE * DVE / 4.0; // area 1/4 exhaust valve
	//xVE = (Raiz2 * c_val - 4.0 / 3.0 * (Raiz2) / Constantes::pi  * (DVE / 2.0)) * cos(Constantes::pi / 4.0);
	yVE = (Raiz2 * c_val - 4.0 / 3.0 * (Raiz2) / Constantes::pi * (DVE / 2.0)) * sin(Constantes::pi / 4.0);
	// Intake valve //valvula de admision
	SVA = Constantes::pi / 4.0 * DVA * DVA / 4.0;
	//xVA = (Raiz2 * c_val - 4.0 / 3.0 * (Raiz2) / Constantes::pi  * (DVA / 2.0)) * cos(Constantes::pi / 4.0);
	yVA = (Raiz2 * c_val - 4.0 / 3.0 * (Raiz2) / Constantes::pi  * (DVA / 2.0)) * sin(Constantes::pi / 4.0);
	//resto del sector cuadrado central (escape)
	SCE = STotal - Siny - SVE; // area 1/4 central squared node after deducting the quarter of injector and valve
	//xCE = (xtotal * STotal - xiny * Siny - xVE * SVE) / SCE; // not used
	yCE = (Ytotal * STotal - yiny * Siny - yVE * SVE) / SCE;
	//resto del sector cuadrado central (admision)
	SCA = STotal - Siny - SVA;
	//xCA = (xtotal * STotal - xiny * Siny - xVA * SVA) / SCA; // not used
	yCA = (Ytotal * STotal - yiny * Siny - yVA * SVA) / SCA;
	//sector rectangular central entre valvulas (2 valvulas - adm+esc -)
	SCentral1 = SCE + SCA; // area 1/2 central squared node after deducting half of injector and a quarter of valves
	//sector rectangular central entre valvulas  (4 valvulas - 2adm+2esc -)
	SCentral = 2.0 * SCentral1; // area central node
	xCentral = (SCE * yCE - SCA * yCA) / SCentral1; // X coordinate of the GC central node
	yCentral = 0.0; // Y coordinate of the GC central node


	// Nodes at the surface of the firedeck: areas and geometric centers
	...
	ST1 = L_per * L_per / 2.0; // area triangle 1/8 firedeck
	ST2 = c_val * c_val / 2.0; // area triangle 1/8 of central squared node
	SC3E = 3.0 / 8.0 * Constantes::pi * DVE * DVE / 4.0; // 3/8 * valve area
	SC3A = 3.0 / 8.0 * Constantes::pi * DVA * DVA / 4.0;
	SEE = ST1 - ST2 - SC3E; // area node firedeck material
	SAA = ST1 - ST2 - SC3A;

	xT1 = 2.0 / 3.0 * L_per;
	xT2 = 2.0 / 3.0 * c_val;
	xC3E = c_val + 4.0 / 3.0 * sin(3.0 * Constantes::pi / 4.0) / (3.0 * Constantes::pi / 4.0) * (DVE / 2.0) * sin((Constantes::pi / 2.0 + Constantes::pi / 4.0) / 2.0);
	xC3A = c_val + 4.0 / 3.0 * sin(3.0 * Constantes::pi / 4.0) / (3.0 * Constantes::pi / 4.0) * (DVA / 2.0) * sin((Constantes::pi / 2.0 + Constantes::pi / 4.0) / 2.0);
	xEE = (xT1 * ST1 - xT2 * ST2 - xC3E * SC3E) / SEE;
	xAA = (xT1 * ST1 - xT2 * ST2 - xC3A * SC3A) / SAA;

	yT1 = 1.0 / 3.0 * L_per;
	yT2 = 1.0 / 3.0 * c_val;
	yC3E = c_val - 4.0 / 3.0 * sin(3.0 * Constantes::pi / 4.0) / (3.0 * Constantes::pi / 4.0) * (DVE / 2.0) * cos((Constantes::pi / 2.0 + Constantes::pi / 4.0) / 2.0);
	yC3A = c_val - 4.0 / 3.0 * sin(3.0 * Constantes::pi / 4.0) / (3.0 * Constantes::pi / 4.0) * (DVA / 2.0) * cos((Constantes::pi / 2.0 + Constantes::pi / 4.0) / 2.0);
	...
	// firedeck is symmetric along Y axis


	// Nodes for intake and exhaust ports: areas and geometric centers	//	[CALMEC]
	// Surface areas
	//cambios 20/04/2012
	if ((_Inicializar.ConfMotor_CULATA_A_PIPA_ADM == 0) | (_Inicializar.ConfMotor_CULATA_A_PIPA_ADM == Constantes::ValorNulo)) { //(JS) added "ValorNulo" case
		SP1_A = Constantes::pi * DVA * (dp / 2.0 - c_val - DVA / 2.0);	// vertical cylinder
		SP2_A = Constantes::pi * DVA * (dp / 2.0 - c_val - DVA / 2.0);	// lateral cylinder
			//SP1_A = Constantes::pi * DVA * (dp / 2.0 - c_val);	//(JS) 2012-Feb change of cylinder length was not included
			//SP2_A = Constantes::pi * DVA * (dp / 2.0 - c_val);	//(JS) 2012-Feb change of cylinder length was not included
		SP_A = SP1_A + SP2_A;
	}
	else {
	...
	if ((_Inicializar.ConfMotor_CULATA_A_PIPA_ESC == 0) | (_Inicializar.ConfMotor_CULATA_A_PIPA_ESC == Constantes::ValorNulo)) { //(JS) added "ValorNulo" case
		SP1_E = Constantes::pi * DVE * (dp / 2.0 - c_val - DVE / 2.0);	// vertical cylinder
		SP2_E = Constantes::pi * DVE * (dp / 2.0 - c_val - DVE / 2.0);	// lateral cylinder
			//SP1_E = Constantes::pi * DVE * (dp / 2 - c_val);	//(JS) 2012-Feb change of cylinder length was not included
			//SP2_E = Constantes::pi * DVE * (dp / 2 - c_val);	//(JS) 2012-Feb change of cylinder length was not included
		SP_E = SP1_E + SP2_E;
	}
	else {
	...
	// Geometric centers
	xP1_A = -c_val;
	xP2_A = -(L_per - (dp / 2.0 - c_val - DVA / 2.0) / 2.0);
		//xP2_A = -(L_per - (dp / 2.0 - c_val) / 2.0);	//(JS) 2012-Feb change of cylinder length was not included
	xP_A = (xP1_A * SP1_A + xP2_A * SP2_A) / SP_A;
	xP1_E = c_val;
	xP2_E = L_per - (dp / 2.0 - c_val - DVE / 2.0) / 2.0;
		//xP2_E = L_per - (dp / 2.0 - c_val) / 2.0;	//(JS) 2012-Feb change of cylinder length was not included
	...
	zP1_A = (dp / 2.0 - c_val - DVA / 2.0) / 2.0 + espesor_FD;
		//zP1_A = (dp / 2.0 - c_val) / 2.0 + espesor_FD;	//(JS) 2012-Feb change of cylinder length was not included
	zP2_A = dp / 2 - c_val + espesor_FD;
	zP_A = (zP1_A * SP1_A + zP2_A * SP2_A) / SP_A;
	zP1_E = (dp / 2.0 - c_val - DVE / 2.0) / 2.0 + espesor_FD;
		//zP1_E = (dp / 2.0 - c_val) / 2.0 + espesor_FD;	//(JS) 2012-Feb change of cylinder length was not included
	zP2_E = dp / 2.0 - c_val + espesor_FD;
	zP_E = (zP1_E * SP1_E + zP2_E * SP2_E) / SP_E;



	// Node loop to fill names and ID in _NODOS_CUL
	for (cont = 1; cont <= 35; cont++) {
		...
		{
		case 1:
			_NODOS_CUL.at(cont).NOMBRE = "CUL1_EAS_A";	// Firedeck nodes at 1st level (surface)
			// 1st letter	E: on the exhaust side		A: on the intake side
			// 2nd letter	E: in contact with exhaust side		A: in contact with intake side
			// 3rd letter	S: y > 0	I: y < 0
			// 4th letter	A: 1st level (surface)	B: 2nd level (above A)
			break;
		case 2:
		...
		case 9:
			...
			break;
		case 10:
			_NODOS_CUL.at(cont).NOMBRE = "CUL10_INY_A";	// Injector part at 1st level
			break;
		case 11:
			_NODOS_CUL.at(cont).NOMBRE = "CUL11_EAS_B";	// Firedeck nodes at 2nd level
			break;
		case 12:
		...
		case 19:
			...
			break;
		case 20:
			_NODOS_CUL.at(cont).NOMBRE = "CUL20_INY_B";	// Injector part at 2nd level
			break;
		case 21:
			_NODOS_CUL.at(cont).NOMBRE = "CUL21_VLV_AS";	// Intake valve 1
			break;
		case 22:
			_NODOS_CUL.at(cont).NOMBRE = "CUL22_VLV_AI";	// Intake valve 2
			break;
		case 23:
			_NODOS_CUL.at(cont).NOMBRE = "CUL23_VLV_ES";	// Exhaust valve 1
			break;
		case 24:
			_NODOS_CUL.at(cont).NOMBRE = "CUL24_VLV_EI";	// Exhaust valve 2
			break;
		case 25:
			_NODOS_CUL.at(cont).NOMBRE = "CUL25_VAST_AS";	// Intake valve stem 1
			break;
		case 26:
			_NODOS_CUL.at(cont).NOMBRE = "CUL26_VAST_AI";	// Intake valve stem 2
			break;
		case 27:
			_NODOS_CUL.at(cont).NOMBRE = "CUL27_VAST_ES";	// Exhaust valve stem 1
			break;
		case 28:
			_NODOS_CUL.at(cont).NOMBRE = "CUL28_VAST_EI";	// Exhaust valve stem 2
			break;
		case 29:
			_NODOS_CUL.at(cont).NOMBRE = "CUL29_INY";	// Injector top part 
			break;
		case 30:
			_NODOS_CUL.at(cont).NOMBRE = "CUL30_PIP_AS";	// Intake port 1
			break;
		case 31:
			_NODOS_CUL.at(cont).NOMBRE = "CUL31_PIP_AI";	// Intake port 2
			break;
		case 32:
			_NODOS_CUL.at(cont).NOMBRE = "CUL32_PIP_ES";	// Exhaust port 1
			break;
		case 33:
			_NODOS_CUL.at(cont).NOMBRE = "CUL33_PIP_EI";	// Exhaust port 2
			break;
		case 34:
			_NODOS_CUL.at(cont).NOMBRE = "CUL34_MA";	// Cyl.-head material intake side
			break;
		case 35:
			_NODOS_CUL.at(cont).NOMBRE = "CUL35_ME";	// Cyl.-head material exhaust side
			break;
	...
	// Coordinates of geometric centers in _NODOS_CUL
	// From node 1 to node 20: firedeck two levels, coordinates X and Y
	for (cont = 1; cont <= 2; cont++) {
		...
		_NODOS_CUL.at(10 * (cont - 1) + 10).x = 0.0;
		...
		_NODOS_CUL.at(10 * (cont - 1) + 10).Y = 0.0;
	}
	// Coordinate Z from node 1 to node 10: Firedeck 1st level
	for (cont = 1; cont <= 10; cont++) {
		_NODOS_CUL.at(cont).Z = espesor_FD / 2.0 / 2.0;
	}
	// Coordinate Z from node 11 to node 20: Firedeck 2nd level
	for (cont = 11; cont <= 20; cont++) {
		_NODOS_CUL.at(cont).Z = espesor_FD / 2.0 + espesor_FD / 2.0 / 2.0;
	}

	// Valves, stems and injector: coordinates X and Y
	...
	_NODOS_CUL.at(29).Y = 0.0;

	// Coordinate Z valves (like firedeck 1st level)
	for (cont = 21; cont <= 24; cont++) {
		_NODOS_CUL.at(cont).Z = espesor_FD / 2.0 / 2.0;
	}
	// Coordinate Z stems
	//'Febrero 2012: dia 17: signos
	for (cont = 25; cont <= 26; cont++) {	// Intake
		_NODOS_CUL.at(cont).Z = (dp / 2.0 - c_val - DVA / 2.0) / 2.0 + espesor_FD;
	}
	for (cont = 27; cont <= 28; cont++) {	// Exhaust
		_NODOS_CUL.at(cont).Z = (dp / 2.0 - c_val - DVE / 2.0) / 2.0 + espesor_FD;
	}
		//For cont = 25 To 28
		//    _NODOS_CUL.at(cont).Z = espesor_FD / 2.0 + Espesor_zona1_2 / 2.0
		//Next cont
		//_NODOS_CUL.at(29).Z = espesor_FD + Espesor_zona1_2 / 2.0

	// Coordinate Z injector (like intake valve stems)
	_NODOS_CUL.at(29).Z = (dp / 2.0 - c_val - DVA / 2.0) / 2.0 + espesor_FD;
	...
	// Cyl.-head material
	_NODOS_CUL.at(34).x = -(L_per / 2.0 - DINY / 2.0);
	_NODOS_CUL.at(35).x = (L_per / 2.0 - DINY / 2.0);

	_NODOS_CUL.at(34).Y = 0.0;
	_NODOS_CUL.at(35).Y = 0.0;

	_NODOS_CUL.at(34).Z = Espesor_zona1_2 / 2.0 + espesor_FD;
	_NODOS_CUL.at(35).Z = Espesor_zona1_2 / 2.0 + espesor_FD;



	// CONTACT AREA WITH CHAMBER GAS //Superficies contacto GAS cilindro
	// Firedeck material
	double pot1 = Constantes::pi * dp * dp / 4.0;	// total contact area
	for (cont = 1; cont <= 4; cont++) {	// Exhaust
		_NODOS_CUL.at(cont).AREA_GAS = SEE - (Lcul * Lcul - pot1) / 8.0;
	}
	for (cont = 5; cont <= 8; cont++) {	// Intake
		_NODOS_CUL.at(cont).AREA_GAS = SAA - (Lcul * Lcul - pot1) / 8.0;
	}
	// Central node
	_NODOS_CUL.at(9).AREA_GAS = SCentral;
	// Injector
	_NODOS_CUL.at(10).AREA_GAS = Constantes::pi * DINY * DINY / 4.0;
	// Intake valves //'valvulas ADMISION
	for (cont = 21; cont <= 22; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = Constantes::pi * DVA * DVA / 4.0;
	}
	// Exhaust valves //'valvulas Escape
	for (cont = 23; cont <= 24; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = Constantes::pi * DVE * DVE / 4.0;
	}

	// CONTACT AREA WITH INTAKE GAS //Superficies contacto GAS admisión
	// Intake valve //Valvula
	for (cont = 21; cont <= 22; cont++) {
		_NODOS_CUL.at(cont).AREA_ADM = Constantes::pi * DVA * DVA * (1.0 - F_VAST_ADM * F_VAST_ADM) / 4.0;
	}
	// Intake valve stems //Vastago
	for (cont = 25; cont <= 26; cont++) {
		_NODOS_CUL.at(cont).AREA_ADM = Constantes::pi * (F_VAST_ADM * DVA) * (dp / 2.0 - c_val - DVA / 2.0 + espesor_FD / 2.0);
	}
		//For cont = 25 To 26
		//    _NODOS_CUL.at(cont).AREA_ADM = Constantes::pi * (F_VAST_ADM * DVA) * (dp - c_val + espesor_FD / 2.0 + DVA / 2.0)
		//Next cont
	// Intake ports //Pipas
	...
	// CONTACT AREA WITH EXHAUST GAS //Superficies contacto GAS escape
	// Exhaust valve //Valvula
	for (cont = 23; cont <= 24; cont++) {
		_NODOS_CUL.at(cont).AREA_ESC = Constantes::pi * DVE * DVE * (1.0 - F_VAST_ESC * F_VAST_ESC) / 4.0;
	}
	// Exhaust valve stems //Vastago
	//Febrero 2012
	for (cont = 27; cont <= 28; cont++) {
		_NODOS_CUL.at(cont).AREA_ESC = Constantes::pi * (F_VAST_ESC * DVE) * (dp / 2.0 - c_val - DVE / 2.0 + espesor_FD / 2.0);
	}
		//For cont = 27 To 28
	...
	// CONTACT AREA WITH COOLANT //'Superficies contacto COOLANT
	//(JS) Some areas may be underestimated or overestimated if port thickness (espesor_pipas) is large
	//cambios 27/01/2014
	// Intake side //Admision 'Modificadas para nuevo conducto en la culata:
	_NODOS_CUL.at(15).AREA_COOL = 0.5 * (L_per - c_val) * (L_per - c_val) - Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas) / 8.0;	// AEI
	_NODOS_CUL.at(16).AREA_COOL = (Lcul * Lcul / 4.0 - c_val * c_val - 3.0 / 4.0 * Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas)) / 2.0;	// AAI	// one quarter/2
	_NODOS_CUL.at(17).AREA_COOL = _NODOS_CUL.at(16).AREA_COOL;	// AAS
	_NODOS_CUL.at(18).AREA_COOL = _NODOS_CUL.at(15).AREA_COOL;	// AES
	_NODOS_CUL.at(30).AREA_COOL = Constantes::pi * (DVA / 2.0 + espesor_pipas) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas);	// Intake port
		// half the perimeter * 2/3 height from firedeck to port = 1/2 * 2 * pi * R * height
	_NODOS_CUL.at(31).AREA_COOL = _NODOS_CUL.at(30).AREA_COOL;	// Intake port
	// Cyl.-head material on the intake side
	_NODOS_CUL.at(34).AREA_COOL = Lcul * L_per + 2.0 * (L_per * Espesor_zona1_2 - (L_per - c_val) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas))
			// half area top (one side, intake) + laterals in contact with the other cylinders (2) - section of coolant gallery
			+ Lcul * Espesor_zona1_2 - 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas) * Lcul - 2.0 * Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas)
			// lateral area in contact with the intake manifold - section of intake ports - section of coolant gallery
			//(JS) Port thickness was not taken into account for the ports section
			//+ Lcul * Espesor_zona1_2 - 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas) * Lcul - 2.0 * Constantes::pi * DVA * DVA / 4.0
			+(L_per - c_val) * Lcul - Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas)
			// ground section - 2 * 1/2 * one port section
			+ (Lcul - 2.0 * DVA - 4.0 * espesor_pipas) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas);
			// vertical sections between intake ports going up from the firedeck
			//(JS) Port thickness was not taken into account for the cooled length
			//+ (Lcul - 2.0 * DVA) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas);

	// Exhaust side //Escape 'Modificadas para nuevo conducto en la culata:
	_NODOS_CUL.at(11).AREA_COOL = 0.5 * (L_per - c_val) * (L_per - c_val) - Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas) / 8.0;	// EAS
	_NODOS_CUL.at(12).AREA_COOL = (Lcul * Lcul / 4.0 - c_val * c_val - 3.0 / 4.0 * Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas)) / 2.0;	// EES
	_NODOS_CUL.at(13).AREA_COOL = _NODOS_CUL.at(12).AREA_COOL;	// EEI
	_NODOS_CUL.at(14).AREA_COOL = _NODOS_CUL.at(11).AREA_COOL;	// EAI
	_NODOS_CUL.at(32).AREA_COOL = Constantes::pi * (DVE / 2.0 + espesor_pipas) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas);	// Exhaust port
	_NODOS_CUL.at(33).AREA_COOL = _NODOS_CUL.at(32).AREA_COOL;	// Exhaust port
	// Cyl.-head material on the exhaust side
	_NODOS_CUL.at(35).AREA_COOL = Lcul * L_per + 2.0 * (L_per * Espesor_zona1_2 - (L_per - c_val) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas))
		+ Lcul * Espesor_zona1_2 - 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas) * Lcul - 2.0 * Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas)
			//(JS) Port thickness was not taken into account for the ports section
			//+ Lcul * Espesor_zona1_2 - 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas) * Lcul - 2.0 * Constantes::pi * DVE * DVE / 4.0
			+ (L_per - c_val) * Lcul - Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas)
			+ (Lcul - 2.0 * DVE - 4.0 * espesor_pipas) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas);
			//(JS) port thickness was not taken into account for the cooled length
			//+ (Lcul - 2.0 * DVE) * 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas);

		//Admision
		//_NODOS_CUL.at(34).AREA_COOL = L_per * L_per - 1.0 / 2.0 * (Constantes::pi * (DINY / 2.0) ^ 2) - 2.0 * (Constantes::pi * (DVA / 2.0) ^ 2)
		//'Escape
		//_NODOS_CUL.at(35).AREA_COOL = L_per * L_per - 1.0 / 2.0 * (Constantes::pi * (DINY / 2.0) ^ 2) - 2.0 * (Constantes::pi * (DVE / 2.0) ^ 2)
		//Febrero 2012
		//_NODOS_CUL.at(34).AREA_COOL = Lcul * Lcul + 2.0 * L_per * Espesor_zona1_2 + Lcul * Espesor_zona1_2 - 2.0 * Constantes::pi / 4.0 * DVA ^ 2 'Febrero 2012
		//Escape
		//_NODOS_CUL.at(35).AREA_COOL = Lcul * Lcul + 2.0 * L_per * Espesor_zona1_2 + Lcul * Espesor_zona1_2 - 2.0 * Constantes::pi / 4.0 * DVE ^ 2 'Febrero 2012



	// Loop of all nodes
	for (cont = 1; cont <= 35; cont++) {
		if (_NODOS_CUL.at(cont).AREA_COOL < 0) {	// Check that contact area with coolant is >= 0
			_NODOS_CUL.at(cont).AREA_COOL = 0.0;
		}
		_NODOS_CUL.at(cont).k = k_cul;	// CONDUCTIVITY
	}
	...
	// Firedeck
	for (cont = 1; cont <= 2; cont++) {	// levels A and B
		for (cont1 = 1; cont <= 4; cont++) {	// Firedeck material on the exhaust side
			_NODOS_CUL.at(cont1 + 10 * (cont - 1)).volume = espesor_FD / 2.0 * (Lcul * Lcul / 8.0 - c_val * c_val / 8.0
				- 3.0 / 8.0 * Constantes::pi * DVE * DVE / 4.0);
		}
		for (cont1 = 1; cont <= 4; cont++) {	// Firedeck material on the intake side
			_NODOS_CUL.at(4 + cont1 + 10 * (cont - 1)).volume = espesor_FD / 2.0 * (Lcul * Lcul / 8.0 - c_val * c_val / 8.0
				- 3.0 / 8.0 * Constantes::pi * DVA * DVA / 4.0);
		}
		_NODOS_CUL.at(9 + 10 * (cont - 1)).volume = espesor_FD / 2.0 * SCentral;	// Firedeck central node
		_NODOS_CUL.at(10 + 10 * (cont - 1)).volume = espesor_FD / 2.0 * Constantes::pi * DINY * DINY / 4.0;	// Injector in firedeck
	}
	
	// Valves
	for (cont1 = 1; cont <= 4; cont++) {
		_NODOS_CUL.at(20 + cont1).volume = espesor_FD / 2.0 * _NODOS_CUL.at(20 + cont1).AREA_GAS;
	}

	// Valve stems
	_NODOS_CUL.at(25).volume = Constantes::pi * F_VAST_ADM * F_VAST_ADM * DVA * DVA / 4.0 * (dp / 2.0 - c_val - DVA / 2.0);	// intake
	_NODOS_CUL.at(26).volume = _NODOS_CUL.at(25).volume;
	_NODOS_CUL.at(27).volume = Constantes::pi * F_VAST_ESC * F_VAST_ESC * DVE * DVE / 4.0 * (dp / 2.0 - c_val - DVE / 2.0);	// exhaust
	_NODOS_CUL.at(28).volume = _NODOS_CUL.at(27).volume;

	// Rest of injector
	_NODOS_CUL.at(29).volume = Constantes::pi * DINY * DINY / 4 * (dp / 2.0 - c_val - DVA / 2.0);

	// Area around ports
	_NODOS_CUL.at(30).volume = Constantes::pi * ((DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas) - (DVA * DVA / 4.0)) * (dp / 2.0 - c_val - DVA / 2.0) * 2.0;
		// intake	// section * length * 2 stretches
	_NODOS_CUL.at(31).volume = _NODOS_CUL.at(30).volume;
	_NODOS_CUL.at(32).volume = Constantes::pi * ((DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas) - (DVE * DVE / 4.0)) * (dp / 2.0 - c_val - DVE / 2.0) * 2.0;
		// exhaust	// section * length * 2 stretches
	_NODOS_CUL.at(33).volume = _NODOS_CUL.at(32).volume;

	// Cyl.-head material
	// Total zone 1_2 (cyl.-head excluding firedeck) - half of upper injector - port volumes (including nearby material) - coolant galleries
	// Intake
	_NODOS_CUL.at(34).volume = Lcul * L_per * Espesor_zona1_2 - _NODOS_CUL.at(29).volume / 2.0
		- 2.0 * Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas) * (dp / 2.0 - c_val - DVA / 2.0) * 2.0
			// 2 pipes * section * length * 2 stretches
			- 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas) * (Lcul * (L_per - c_val) - Constantes::pi * (DVA / 2.0 + espesor_pipas) * (DVA / 2.0 + espesor_pipas));
			// coolant gallery volume = height * ground area
			// height = 2.0 / 3.0 * (dp / 2.0 - c_val - DVA / 2.0 - espesor_pipas)   // 2/3 height from firedeck to port
			// ground area = side Lcul * side (L_per - c_val) - 2.0 * port sections / 2.0
	// Exhaust
	_NODOS_CUL.at(35).volume = Lcul * L_per * Espesor_zona1_2 - _NODOS_CUL.at(29).volume / 2.0
		- 2.0 * Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas) * (dp / 2.0 - c_val - DVE / 2.0) * 2.0
		- 2.0 / 3.0 * (dp / 2.0 - c_val - DVE / 2.0 - espesor_pipas) * (Lcul * (L_per - c_val) - Constantes::pi * (DVE / 2.0 + espesor_pipas) * (DVE / 2.0 + espesor_pipas));
...
void TransmisionCalor::calcular_conductancias_conductivas()
{
	...
	ncilin = (_NODOS_CIL).size() - 1;	// First line contains no data - it is used to start rows from 1 instead of from 0
	...
	d = sqrt((_NODOS_PIS.at(n1).X_CG - _NODOS_PIS.at(n2).X_CG) * (_NODOS_PIS.at(n1).X_CG - _NODOS_PIS.at(n2).X_CG)
		+ (_NODOS_PIS.at(n1).Y_CG - _NODOS_PIS.at(n2).Y_CG) * (_NODOS_PIS.at(n1).Y_CG - _NODOS_PIS.at(n2).Y_CG));
	...
	Lmaxim = _NODOS_CIL.at(1).L_SUP;	// distance of top dead center from cylinder bottom

	// Obtain all distances from the firedeck to the position of the piston ring over time
	ang = 0.0;
	for (cont1 = 1; cont1 <= filas_total; cont1++) {
		...
		spiston.at(cont1) = _Inicializar.ConfMotor_PISTON_HLI + dist1;	// distance from the firedeck to the position of the piston top
		pos_seg.at(cont1) = spiston.at(cont1) + _NODOS_PIS.at(1).ALTURA + _NODOS_PIS.at(2).ALTURA + _NODOS_PIS.at(3).ALTURA / 2;
			// distance from the firedeck to the position of the piston ring (located in the middle of piston node 3)
		ang = ang + 360.0 / filas_total;
	}	// one revolution, 2 strokes
		...
		for (cont2 = 1; cont2 <= nradiales; cont2++) {
			for (cont3 = 1; cont3 <= ncircunf; cont3++) {
				if (cont2 == 1) {
					...
					Kseg = (_NODOS_CIL.at(nodo_cil_seg).ANG_FIN - _NODOS_CIL.at(nodo_cil_seg).ANG_INI) * Constantes::pi / 180.0 * 4.0 * acum / log((1.0 + _NODOS_CIL.at(nodo_cil_seg).D_MAX / _NODOS_CIL.at(nodo_cil_seg).D_MIN) / 2.0);
	...
	double raiz2;
	double Kseat;
	//Proporción entre el diámetro de la válvula y el diamétro del vástago
	double F_VAST_ADM;
	double F_VAST_ESC;
	double C_DINY;
...
double TransmisionCalor::conductancia_radial(double k, double l, double r1, double r2, double FI)
{
	...
	//la conductividad (k)
	//la longitud o altura del cilindro (L)
	//los radios interiores y exteriores (r1 y r2)
	//el ángulo considerado (fi en rad)
	///'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
{
	return sqrt((_NODOS_CUL.at(nodo1).x - _NODOS_CUL.at(nodo2).x) * (_NODOS_CUL.at(nodo1).x - _NODOS_CUL.at(nodo2).x)
		+ (_NODOS_CUL.at(nodo1).Y - _NODOS_CUL.at(nodo2).Y) * (_NODOS_CUL.at(nodo1).Y - _NODOS_CUL.at(nodo2).Y)
		+ (_NODOS_CUL.at(nodo1).Z - _NODOS_CUL.at(nodo2).Z) * (_NODOS_CUL.at(nodo1).Z - _NODOS_CUL.at(nodo2).Z));
}
...

{
	...
	*ncircunf = _Inicializar.ConfMotor_BASE_NODOS_CIRCUNF;
	ngases = *naxiales - 1;
	nodos_conv = _NODOS_FLUIDOS.size() - 1 - ngases;
	ncilin = *naxiales * *nradiales * *ncircunf + *ncircunf;
	*npis = _NODOS_PIS.size() - 1;
	*ncul = _NODOS_CUL.size() - 1;
	...
	// Resize vector for vertical lengths of axial nodes
	L_nodos.resize(*naxiales + 1); //Aqui almaceno las longitudes de cada uno de los nodos axiales

	// Resize conductance vectors
	...
	// Extract conductive conductances from _CONDUCTANCIAS_CONDUCT and store them in 2-dim conductance vectors
	int num_nodo1;
	int num_nodo2;
	for (int i = 0; i < _CONDUCTANCIAS_CONDUCT.size(); i++)
	{
		// Refer node numbers to the position of the first liner node
		...
		if ((num_nodo1 > 0) && (num_nodo1 <= ncilin)) //nodo del cilindro
		{
			...
			// LINER - PISTON
			else if (((num_nodo2 - ncilin) > 0) && ((num_nodo2 - ncilin) <= *npis)) //relacionado con nodo del piston
				//(JS) changed from if to else if; num_nodo2 can not be both a liner node and a piston node
			{
				k_cil_pis->at(num_nodo1).at(num_nodo2 - ncilin) = _CONDUCTANCIAS_CONDUCT.at(i).k;
				k_pis_cil->at(num_nodo2 - ncilin).at(num_nodo1) = _CONDUCTANCIAS_CONDUCT.at(i).k;	// symmetric
			}
			// LINER - CYL.-HEAD (no contact)
		...
		{// PISTON or CYL.-HEAD
			// Adjust num_nodo1 and num_nodo2 to use them directly in k_pis
			...
			{	// CYL.-HEAD
				// Adjust num_nodo1 and num_nodo2 to use them directly in k_cul
