...
void TTubo::UpdateInitialParameters(double p, double T, double u) {
...
void TTubo::LeeDatosGeneralesTuboXML(xml_node node_pipe, TBloqueMotor **Engine) {
	...
	try {
		...
		if(TypeScheme == "LaxWendroff") {
			...
			if(FCT) {
				FMod.SubModelo = nmFCT;
				std::string FCT_Opt = node_nummet.attribute("FCT_Opt").value();
				if(FCT_Opt == "DDNAD") {
					FMod.OpcionSubModelo = nmDDNAD;
					FMod.Difusion = nmDamping;
					FMod.Antidifusion = nmNaive;
...
void TTubo::LeeDatosGeometricosTuboXML(xml_node node_pipe, double ene, int tipomallado) {
	...
	try {
		...
		int id = 0;
		for(xml_node node_stretch = GetNodeChild(node_geometry, "Geo_Stretch"); node_stretch;
			node_stretch = node_stretch.next_sibling("Geo_Stretch")) {
			id = GetAttributeAsInt(node_stretch, "Stretch_ID");
			if(id > FNTramos) {
				std::cout << "ERROR: The stretches of the pipe are not correctly ordered in pipe: " << FNumeroTubo << std::endl;
				throw Exception("");
			}
			...
			if(CC[FNodoIzq - 1]->getTipoCC() == nmPipeToPlenumConnection) {
				...
				for(int k = 0; k < Deposito[numDeposito - 1]->getNUniones(); k++) {
					if(Deposito[numDeposito - 1]->GetCCDeposito(k)->getUnionDPF() && !PrimeraVez) {
						...
						if(Deposito[numDeposito - 1]->GetCCDeposito(k)->GetTuboExtremo(0).TipoExtremo == nmLeft) {
							FNodoDPFEntrada = 0;
						} else {
...
void TTubo::Transforma3Area(double **Ufct, double **U, double Area, double Gamma, double Gamma1, double Gamma6, int i) {
	...
	try {
		...
		Ufct[2][i] = ((U[2][i] - pow2(U[1][i]) / U[0][i] / 2.) * Gamma1)
					 * pow((1. + (Gamma1 / 2.) * pow2(U[1][i]) / U[0][i] / U[0][i] / (Gamma * ((U[2][i] - pow2(
...
void TTubo::TransformaContorno(double& L, double& B, double& E, double& a, double& v, double& p, const int& modo,
							   const double& Gamma1, const double& Gamma3, const double& Gamma4,
							   const double& Gamma5) {
#ifdef usetry
	try {
#endif
...
void TTubo::ReadAverageResultsTuboXML(xml_node node_pipe) {
	...
	try {
		...
		for(xml_node node_point = GetNodeChild(node_res, "Opi_Point"); node_point;
			...
			for(xml_attribute parameter = node_point.attribute("parameter"); parameter; parameter = parameter.next_attribute()) {
	...
	try {
		...
		for(int i = 0; i < FNumResMedios; i++) {
			...
			if(ResultadosMedios[i].TemperaturaGas) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4005) + PutLabel(910) + "/" + PutLabel(3041);
			...
			if(ResultadosMedios[i].TemperaturaInternaPared) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4005) + PutLabel(910) + "/" + PutLabel(3043) + "/" + PutLabel(3011);
				medoutput << Label.c_str();
			}
			if(ResultadosMedios[i].TemperaturaIntermediaPared) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4005) + PutLabel(910) + "/" + PutLabel(3043) + "/" + PutLabel(3012);
				medoutput << Label.c_str();
			}
			if(ResultadosMedios[i].TemperaturaExternaPared) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4005) + PutLabel(910) + "/" + PutLabel(3043) + "/" + PutLabel(3013);
				medoutput << Label.c_str();
			}
			...
			if(ResultadosMedios[i].FraccionMasicaEspecies) {
				for(int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
					Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
								4023) + PutLabel(901) + "/" + DatosEspecies[j].Nombre;
					medoutput << Label.c_str();
				}
			}
		}
...
void TTubo::ReadInstantaneousResultsTuboXML(xml_node node_pipe) {
	...
	try {
		...
		xml_node node_res = GetNodeChild(node_pipe, "Pip_InsOutput");
		...
		for(xml_node node_point = GetNodeChild(node_res, "Opi_Point"); node_point;
			...
			for(xml_attribute parameter = node_point.attribute("parameter"); parameter; parameter = parameter.next_attribute()) {
				...
				if(par == "Pressure") {
					...
					ResultInstantaneos[i].FlujoMasico = true;
				} else if(par == "RigthVelocity") {
					ResultInstantaneos[i].VelocidadDerecha = true;
				} else if(par == "LeftVelocity") {
					ResultInstantaneos[i].VelocidadIzquierda = true;
					...
					ResultInstantaneos[i].NIT = true;
				} else if(par == "IntWallTemperature") {
					ResultInstantaneos[i].TemperaturaInternaPared = true;
				} else if(par == "MedWallTemperature") {
					ResultInstantaneos[i].TemperaturaIntermediaPared = true;
				} else if(par == "ExtWallTemperature") {
					ResultInstantaneos[i].TemperaturaExternaPared = true;
				} else if(par == "IntHeatTransCoef") {
					ResultInstantaneos[i].CoefPelInterior = true;
				} else if(par == "SpeciesMassFraction") {
					ResultInstantaneos[i].FraccionMasicaEspecies = true;
				} else if(par == "SpecHeatRatio") {
					ResultInstantaneos[i].Gamma = true;
				} else {
					std::cout << "WARNING: El tipo de variable seleccionada para la salida de resultados instantaneos no es valida" <<
							  std::endl;
	...
	try {
		...
		for(int i = 0; i < FNumResInstant; i++) {
			...
			if(ResultInstantaneos[i].Pressure) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4006) + PutLabel(908) + "/" + PutLabel(3042);
				insoutput << Label.c_str();
			}
			if(ResultInstantaneos[i].Velocity) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4007) + PutLabel(909) + "/" + PutLabel(3026);
				insoutput << Label.c_str();
			}
			...
			if(ResultInstantaneos[i].VelocidadDerecha) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4007) + PutLabel(909) + "/" + PutLabel(3031);
				insoutput << Label.c_str();
			}
			if(ResultInstantaneos[i].VelocidadIzquierda) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4007) + PutLabel(909) + "/" + PutLabel(3030);
				insoutput << Label.c_str();
			}
			if(ResultInstantaneos[i].PresionDerecha) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4006) + PutLabel(908) + "/" + PutLabel(3031);
				insoutput << Label.c_str();
			}
			if(ResultInstantaneos[i].PresionIzquierda) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4006) + PutLabel(908) + "/" + PutLabel(3030);
				insoutput << Label.c_str();
			}
			...
			if(ResultInstantaneos[i].CoefPelInterior) {
				Label = "\t" + PutLabel(5004) + "/" + std::to_string(FNumeroTubo) + "/" + TextDist.str() + "m/" + PutLabel(
							4015) + PutLabel(911) + "/" + PutLabel(3011);
				insoutput << Label.c_str();
			}
					...
					if(FHayDPFNodoIzq) {
						if(FTipoCanal[0] == 0) {
							...
							FResistAxiAnt[i] = FDPFEntradaTubo->getAjustRAxAnt()
											   / (__cons::Pi * FDPFEntradaTubo->getConductividadMetal()
												  * (FDPFEntradaTubo->getDiametroEfect() + 2 * FDPFEntradaTubo->getEspesorAislante() + 2 *
													 FDPFEntradaTubo->getEspesorAire() + FDPFEntradaTubo->getEspesorMetal())
						...
						if(FHayDPFNodoDer) {
							if(FTipoCanal[1] == 0) {
								...
								FResistAxiPos[i] = FDPFSalidaTubo->getAjustRAxAnt()
												   / (__cons::Pi * FDPFSalidaTubo->getConductividadMetal()
													  * (FDPFSalidaTubo->getDiametroEfect() + 2 * FDPFSalidaTubo->getEspesorAislante() + 2 * FDPFSalidaTubo->getEspesorAire()
			...
			for(int i = 0; i < FNin; i++) {
				...
				if(i == 0) {
					if(BC[FNodoIzq - 1]->getTipoCC() == nmPipesConnection) {
						...
						if(FHayDPFNodoIzq) {
							Tpantant = __units::degCToK(FDPFEntradaTubo->GetTSuperficie(FNodoDPFEntrada, 2));
						}
					}
#endif
				...
				if(FResistAxiAnt[i] > 0. && FResistAxiPos[i] > 0.) {
					FTPTubo[1][i] =
						...
						* (1 / FResistRadInt[i] * (Tpant0 - Tpant1) + 1 / FResistRadExt[i] * (Tpant2 - Tpant1) + 1 / FResistAxiAnt[i] *
						   (Tpantant - Tpant1)
						...
						for(int i = 0; i < FNin; i++) {
							...
							if(i == 0) {
								...
								if(BC[FNodoDer - 1]->getTipoCC() == nmPipesConnection) {
									...
									if(FHayDPFNodoDer) {
										Tpantpos = __units::degCToK(FDPFSalidaTubo->GetTSuperficie(FNodoDPFSalida, 2));
									}
								}
#endif
							} else {
							...
							if(EsPrimeraVez) {
								...
								if(FResistAxiAnt[i] > 0. && FResistAxiPos[i] > 0.) {
									FTPTubo[1][i] = (FSUMTime * (1 / (Re + FResistRadExt[i]) * Text + 1 / FResistAxiAnt[i] * Tpantant + 1 / FResistAxiPos[i]
													...
													/ (FSUMTime * (1 / (Re + FResistRadExt[i]) + 1 / FResistAxiAnt[i] + 1 / FResistAxiPos[i]) + FSUMTPTuboPro[1][1][i]);
								} else if(FResistAxiAnt[i] > 0.) {
									FTPTubo[1][i] = (FSUMTime * (1 / (Re + FResistRadExt[i]) * Text + 1 / FResistAxiAnt[i] * Tpantant) +
													 FSUMTPTuboPro[0][1][i])
													/ (FSUMTime * (1 / (Re + FResistRadExt[i]) + 1 / FResistAxiAnt[i]) + FSUMTPTuboPro[1][1][i]);
								} else if(FResistAxiPos[i] > 0.) {
									FTPTubo[1][i] = (FSUMTime * (1 / (Re + FResistRadExt[i]) * Text + 1 / FResistAxiPos[i] * Tpantpos) +
													 FSUMTPTuboPro[0][1][i])
													/ (FSUMTime * (1 / (Re + FResistRadExt[i]) + 1 / FResistAxiPos[i]) + FSUMTPTuboPro[1][1][i]);
								} else {
									FTPTubo[1][i] = (FSUMTime * (1 / (Re + FResistRadExt[i]) * Text) + FSUMTPTuboPro[0][1][i]) / (FSUMTime * (1 /
													(Re + FResistRadExt[i])) + FSUMTPTuboPro[1][1][i]);
								}
