...
void FUNCTION_FOR_INTERPOLATION(double *interpolated, double *time_interpolated, double *CAD_to_interpolate,
								double *vector_to_interpolate, int size_interpolated, int size_to_interpolate,
								double speed) {
	...
	for(counter = 0; counter < size_interpolated; counter++) {
		...
		if(auxiliar == 0) {
			...
			interpolated[counter] = vector_to_interpolate[auxiliar - 1]
									+ (vector_to_interpolate[auxiliar] - vector_to_interpolate[auxiliar - 1]) * (time_interpolated[counter] -
											CAD_to_interpolate[auxiliar - 1] * 60. / (360. * speed))
									/ (CAD_to_interpolate[auxiliar] * 60. / (360. * speed) - CAD_to_interpolate[auxiliar - 1] * 60. / (360. * speed));
...
void CALCULUS_OF_VIRTUAL_VELOCITY(double *inj_velocity, double *virtual_velocity, double *dmf, double *time_vector,
								  double rofuel, double dc, double n_holes, double nozzle_d, double D, int size,
								  double PI, double speed, double *EOI_IM, double inj_num, double *SOI_IM, double Piston_D, double DBowl, double CTM,
...
void CALCULUS_OF_POI(double **POI_IM, double **mfuel_i_IM, double **mfuel_ij_IM, double *acu_dmf, double *time_vector,
					 int size, double speed, int *num_i_IM, int num_j, double *SOI_IM, double* EOI_IM,
...
void CALCULATE_CYCLE(double *roair, double *CAD, double delta_t, double *V_cyl, double VTDC, int counter, double speed,
					 double *p_cyl, double *HRF, double *acu_mf, double *Mbb, double *acu_Mbb,
					 double AFe, double f, double mfuel, double mEGR, double mairIVC, double *T_cyl, double HP, double *Yair, double *Yfuel,
					 double *Yburned, double *U, double *CV, double *H_cooler, double *H,
					 double T_Evaporation_fuel, double inj_fuel_temp, double PRECISION_ITERATION, double *defor, double *Rmixture,
					 double Atmosphere_press, double *Gamma, double PI, double Runiv, double Piston_D,
					 ...
					 double MW_fuel, double MW_burned, double C_ESteel, double C_Mech_Defor, double CTM, double WC1A, double WC1B, double C2,
	...
	if(counter < 2) {
		...
		*Gamma = (CV[counter - 1] + Rmixture[counter - 1]) / CV[counter - 1];
		...
		ecg = -21776.6 * Runiv / MW_fuel
			  + Runiv / MW_fuel
			  * (-4.5826 * T_Evaporation_fuel + 0.12428 / 2. * pow(T_Evaporation_fuel, 2.) - 0.00007233 / 3. * pow(T_Evaporation_fuel,
					  3.) + 0.000000016269 / 4. * pow(T_Evaporation_fuel, 4.)
...
double VOLUME(double CAD, double VTDC, double PI, double Piston_D, double Crank_L, double Connecting_Rod_L, double E) {
	...
	AUX =
		(Crank_L * Sqrt(pow(1. + 1. / (Crank_L / Connecting_Rod_L), 2) - pow(E / Crank_L,
...
void DEFORMATIONS(double *V_cyl, double *DEFOR, double p_cyl, double CAD, double delta_CAD, double speed, double PI,
				  double Piston_D, double S, double Connecting_Rod_L, double E, double Piston_Axis_D,
				  double Piston_Crown_H, double M_Connecting_Rod, double M_P_R_PA, double C_ESteel, double C_Mech_Defor) {
...
double HEAT_COOLER(double p_cyl, double pressureIVC, double T_cyl, double temperatureIVC, double average_Volume,
				   double volumeIVC, double delta_CAD, double speed, double VTDC, double *H, double PI,
				   double Piston_D, double S, double DBowl, double VBowl, double CTM, double WC1A, double WC1B, double C2, double TPIS,
				   double TCYL_HEAD, double TCYL, double CAD, double *Qcylhead, double *Qcyl,
				   double *Qpis, int counter)
...
void CALCULUS_OF_IMP_HP(double *complete_p_cyl, double *complete_CAD, double *p_cyl, double *V_cyl,
						...
						double Cylinder_capacity, double PI, double Piston_D, double S, double Crank_L,
						double Connecting_Rod_L, double E, double Piston_Axis_D, double Piston_Crown_H, double M_Connecting_Rod,
						double M_P_R_PA, double C_ESteel, double C_Mech_Defor, double inlet_pres,
						double exhaust_pres) {
...
void ACT(double *engine_parameters, double *engine_model_constants, double *test_variables, double *injection_rate,
		 double *CAD_injection_rate, int size_inlet_inj, int NIN, double *SOI, double *EOI,
		 int CAI, double *CAD_exit, double *HRF_exit, double *ROHR_exit, double *p_cyl_exit, double *dp_da_cyl_exit,
		 double *T_cyl_exit, double *H_cooler_exit, double *mean_var_exit, double *heat_transfer,
		 double *injection_rate_exit, double *accum_injection_rate_exit, sINtype dataIN, sOUTtype *dataOUT) {
	...
	for(counter = 0; counter < size - 1; counter++) {
		rate_area = rate_area + (time_vector[counter + 1] - time_vector[counter]) * min(dmf[counter], dmf[counter + 1])
					+ (time_vector[counter + 1] - time_vector[counter]) * pow(pow((dmf[counter + 1] - dmf[counter]) / 2, 2), 0.5);
	}
						...
						if((POI_IM[m][i] <= time_vector[counter]) && (element[m][aux].mf_jet > 0) && (mO2_bowl[counter] > 0)
						   && (element[m][aux].state != stOvermixed)) {
							...
							element[m][aux].dmtotal = Kmixture1 * (pow(element[m][aux].mf_jet,
																   2) / element[m][aux].mtotal) * virtual_velocity[counter] * pow(roair[counter], 0.5)
													  * pow(nozzle_d * 1000, -1) * pow((mO2_bowl[counter] / mtotal_bowl[counter]),
							...
							if((element[m][aux].state == stEvaporated) && (time_vector[counter] > (SOC_IM[m] / (6 * speed)))) {
								...
								element[m][aux].Rpmx = element[m][aux].Rpmx
													   + Kpmx1 * pow((mO2_bowl[counter] / mtotal_bowl[counter]), 0.855) * ((1. / 4. - 1.) / pow(log(.1),
															   2) * pow(log(element[m][aux].FI), 2) + 1.)
													   * exp(-Kpmx2 / T_cyl[counter]) * pow(roair[counter], Kpmx5) * pow(speed / 1000., Kpmx3) * pow(PCR / 500.,
															   Kpmx4) * delta_t;
							...
							element[m][aux].dSOOT_A = KSOOTA4 * element[m][aux].mSOOT_A * pow((realelement[m][aux].mO2 / element[m][aux].mtotal),
													  KSOOTA5) * pow(p_cyl[counter], KSOOTA6)
													  * exp(-KSOOTA7 / element[m][aux].Tadib) * delta_t;
							...
							if(element[m][aux].tLOL > -10.) {
								...
								if((time_vector[counter] - element[m][aux].tLOL) > 0.) {
									...
									valor = KSOOTC1 * realelement[m][aux].mf_reac * pow(Pres / (14.8 * Runiv / MW_air * 1000.),
											2.2) * (-1. / element[m][aux].FRLOL + 0.5) * 1.e6
											* exp(-2399. / Tadib_cyl[counter]) * delta_t;
				...
				for(i = 0; i < num_i_IM[m]; i++) {
					for(j = 0; j < num_j; j++) {
						if(POI_IM[m][i] <= time_vector[counter]) {
							...
							if(((element[m][aux].state == stEvaporated) && (time_vector[counter] > (SOC_IM[m] / (6 * speed))))
							   || (element[m][aux].state == stBurned_rich_premix)
							   || (element[m][aux].state == stBurned_poor_premix) || (element[m][aux].state == stBurned_by_diffusion)
							   || (element[m][aux].state == stBurned_by_second_diffusion)
							   || (element[m][aux].state == stInto_diffusion_flame)) {
							...
							if(((element[m][aux].state == stEvaporated) && (time_vector[counter] > (SOC_IM[m] / (6 * speed))))
							   || (element[m][aux].state == stBurned_rich_premix)
							   || (element[m][aux].state == stBurned_poor_premix) || (element[m][aux].state == stBurned_by_second_diffusion)) {
			...
			if(CalcRad[m] == 1 && RadCalc == 1) {   // Solo se calcula si toca y si el usuario quiere
				...
				if(iini < ifin) {
					...
					if(iini > 0) {
						...
						Radiation[m][0].x = (element[m][iini * num_j + 4].X * (element[m][(iini - 1) * num_j + 4].FI - 1.)
											 + element[m][(iini - 1) * num_j + 4].X * (1. - element[m][iini * num_j + 4].FI)) / (element[m][(iini - 1) * num_j +
													 4].FI - element[m][iini * num_j + 4].FI);
						...
						if(Radiation[m][i - iini].RFlame > cero) {
							...
							for(k = 0; k < Radiation[m][i - iini].np; k++) {
								...
								Radiation[m][i - iini].T[k] = Teje
															  + (Radiation[m][0].x / Radiation[m][i - iini].x * (exp(-4.6 * pow(radio / Radiation[m][i - iini].R,
																	  2)) - 1.) / (1. - Radiation[m][0].x / Radiation[m][i - iini].x))
															  * (Tllama - Teje);
