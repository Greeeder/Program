...
void TCCCompresor::LeeDatosCompresorXML(xml_node node_connect, xml_node node_openwam) {
	try {
		...
		std::string Type = node_openwam.child("BlockOfCompressors").find_child_by_attribute("Boc_Compressor", "ID",
						   node_comp.attribute("Compressor_ID").value()).attribute("Type").as_string();
...
void TCCCompresor::AsignacionDatos(TCompresor **Compressor, TDeposito **Plenum, const char *FileWAM, fpos_t &filepos,
								   int NumberOfPipes, TTubo **Pipe, TCondicionContorno **BC, int numCC,
...
void TCCCompresor::ReadCompressorDataXML(TCompresor **Compressor, xml_node node_comp, double AmbientTemperature,
