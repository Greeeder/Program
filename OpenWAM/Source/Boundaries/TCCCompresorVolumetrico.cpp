...
void TCCCompresorVolumetrico::ReadBoundaryDataXML(xml_node node_connect, int NumberOfPipes, TTubo **Pipe, int nDPF,
		TDPF **DPF) {
...
// Inicializacion del transporte de especies quimicas.
		...
		if(FNumeroEspecies == 4 || FNumeroEspecies == 10)
			HayFuel = true;
		ImposeCompositionXML(nod_compini, FComposicion, FHayEGR, HayFuel, FCalculoEspecies);
		for(int i = 0; i < FNumeroEspecies - 1; i++) {
			FFraccionMasicaEspecie[i] = FTuboExtremo[0].Pipe->GetFraccionMasicaInicial(i);
			fracciontotal += FComposicion[i];
		}
...
//FILE *fich=fopen(FileSALIDA,"a");
		...
		if(FResMediosCV.Potencia) {
			Label = "\t" + PutLabel(5010) + "/" + std::to_string(FNumeroCC) + "/" + PutLabel(4009) + PutLabel(903);
			medoutput << Label.c_str();
		}
		if(FResMediosCV.Massflow) {
			Label = "\t" + PutLabel(5010) + "/" + std::to_string(FNumeroCC) + "/" + PutLabel(4008) + PutLabel(904);
			medoutput << Label.c_str();
		}
		if(FResMediosCV.Pressure) {
			Label = "\t" + PutLabel(5010) + "/" + std::to_string(FNumeroCC) + "/" + PutLabel(4006) + PutLabel(908);
			medoutput << Label.c_str();
		}
