...
void TCCDescargaExtremoAbierto::ReadBoundaryDataXML(xml_node node_connect, int NumberOfPipes, TTubo **Pipe, int nDPF,
		TDPF **DPF) {
	try {
		int i = 0;
		double fracciontotal = 0.;
		...
		if(FTipoDescarga == nmDescargaAtmosfera) {     // DESCARGA A LA ATMOSFERA
			...
			if(FNumeroEspecies == 4 || FNumeroEspecies == 10)
				HayFuel = true;
			ImposeCompositionXML(nod_comp, FComposicion, FHayEGR, HayFuel, FCalculoEspecies);
