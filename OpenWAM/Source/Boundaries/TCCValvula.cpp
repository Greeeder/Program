		...
		if(rel_CCon_Entropia / FAd > 1.000005) {   // Flujo entrante al cilindro
			...
			if(FCDEntrada > 0.0001) {  /* Abierto */
				...
				if(FGasto < -1e-5) {
					...
					FMomento = coef * pow2(ARef * FGasto) / (200000 * FCarrera * Gamma * FPresionCil) * (pow2(
								   *FCC + *FCD) / 4. + Gamma3 * pow2((*FCD - *FCC) / Gamma1));
				}
				/*if (i == 0) {
...
//Fin caso de salto supercritico
		...
		FGasto = -fabs(Gamma * FSeccionTubo * pow(FSonido,
					   2 * Gamma6) * FVelocity * 1e5 / (ARef * xaa2)); // Massflow entrante al cilindro negativo
...
//Variables para resolver la onda de choque.
		...
		if(error < 0.) {  // Salto de presiones supercritico.
			...
			FGasto = fabs(FCDSalida * FSeccionValvula * Gamma * pow(FAd / FVelSonidoCil, Gamma4) * FVelocidadGarganta * pow(a1,
						  2. / Gamma1) * 1e5 / ARef);
			xx = *FCC + Gamma3 * FVelocity * FSigno;
			FTuboExtremo[0].Entropia = FTuboExtremo[0].Entropia * FSonido / xx; // Ecuacion de la caracteristica incidente.
		}
...
void TCCCilindro::Resolucion(double ext1, double ext2, nmCaso Caso, double *u2t, double *a2t, double prey,
							 double prex) {
	try {
		int contador = 0;
		...
		while(contador <= 800 && !salida) {
			...
			case nmFlujoSalienteSaltoSupercritico:
				...
				break;
			default:
				printf("Error en la definicion del flujo TCCCilindro::Resolucion en la condicion de contorno: %d\n", FNumeroCC);
				throw Exception("");
			} /*fin del switch*/
...
void TCCCilindro::FESupercritico(double mach_supuesto, double *miembro1, double *miembro2) {
	try {
...
// Resolucion de la ecuacion 4.20 de la tesis de Corberan.
		*a2_2 = sqrt(vel_supuesta * pow((*FCC + Gamma3 * vel_supuesta * FSigno) / FTuboExtremo[0].Entropia,
										Gamma4) / Fcc); // Si vel_supuesta < 0, Fcc tambien lo es.
