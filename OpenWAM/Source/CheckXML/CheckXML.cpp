...
xml_node GetNodeChild(xml_node node, const char* name) {
	if(node.child(name).empty() && (verbosity > 1)) {
		std::cout << "INFO: Child node not found" << std::endl;
...
double GetAttributeAsDouble(xml_node node, const char* name) {
	if(node.attribute(name).empty() && (verbosity > 1)) {
		std::cout << "INFO: Attribute not found" << std::endl;
...
int CountNodes(xml_node node, const char* label) {
	...
	int total = 0;
	for(xml_node node_child = GetNodeChild(node, label); node_child; node_child = node_child.next_sibling(label)) {
		total++;
	}
	return total;
}
...
double GetXMLLength(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
...
double GetXMLMassFlow(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
...
double GetXMLMass(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
	return to_kg(x, unit);
}
...
double GetXMLPressure(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
	return to_bar(x, unit);
}
...
double GetXMLTemperature(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
	return to_celsius(x, unit);
}
...
double GetXMLTime(const xml_node& node, const std::string& name, const std::string& unit) {
	double x = GetAttributeAsDouble(node, name.c_str());
	return to_seconds(x, unit);
}
