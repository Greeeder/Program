...
void TDiscoRotativo::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	try {
		...
		std::string SpeedControl = node_rotary.attribute("SpeedControl").value();
		...
		int j = 0;
		for(xml_node node_flow = GetNodeChild(node_rotary, "RtV_FlowCoef"); node_flow;
			node_flow = node_flow.next_sibling("RtV_FlowCoef")) {
