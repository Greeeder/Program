...
void TLamina::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	try {
		...
		std::string Type = node_reed.attribute("Type").value();
		if(Type == "0D") {
			FTipoLamina = nmLamina0D;
		} else if(Type == "1D") {
			FTipoLamina = nmLamina1D;
		...
		std::string Mode = node_reed.attribute("Mode").value();
		if(Mode == "Positive") {
			FSentidoLamina = 1;
		...
		int j = 0;
		for(xml_node node_dcin = GetNodeChild(node_reed, "RdV_DCin"); node_dcin;
			node_dcin = node_dcin.next_sibling("RdV_DCin")) {
			FLiftCDin[j] = (double) j * FIncrLev;
			FDatosCDEntrada[j] = GetAttributeAsDouble(node_dcin, "DC_in");
			++j;
		}
		j = 0;
		for(xml_node node_dcin = GetNodeChild(node_reed, "RdV_DCout"); node_dcin;
			node_dcin = node_dcin.next_sibling("RdV_DCout")) {
			FLiftCDout[j] = (double) j * FIncrLev;
...
//FILE *fich=fopen(FileSALIDA,"a");
		...
		if(FGraficasLam) {
			if(FGrafLev) {
				Label = "\t" + PutLabel(5015) + "/" + std::to_string(lam) + "/" + PutLabel(4030) + PutLabel(902);
