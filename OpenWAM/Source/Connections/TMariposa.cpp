...
void TMariposa::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	int ctrl, prm;

	FNumeroOrden = norden;

	xml_node node_throttle = GetNodeChild(node_valve, "Val_Throttle");
	std::string LengthUnits = "";

	if (node_throttle.child("Units")){
		xml_node node_units = GetNodeChild(node_throttle, "Units");
		LengthUnits = node_units.attribute("Length").value();
	}
	...
	int i = 0;
	for(xml_node node_lift = GetNodeChild(node_throttle, "Thr_Lift"); node_lift;
		node_lift = node_lift.next_sibling("Thr_Lift")) {
