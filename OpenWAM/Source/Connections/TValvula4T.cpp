...
void TValvula4T::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	try {
		int ControlRegimen, NumLev = 0, NumCD = 0;
		...
		int i = 0;
		for(xml_node node_profile = GetNodeChild(node_4svalve, "V4s_Profile"); node_profile;
		...
		i = 0;
		for(xml_node node_flowcoef = GetNodeChild(node_4svalve, "V4s_FlowCoef"); node_flowcoef;
		...
		std::string SpeedControl = node_4svalve.attribute("SpeedControl").value();
...
void TValvula4T::GetCDin(double Time) {
	...
	if (NewCylConnected) {
		NewCyl->getCrankMechanism()->getCurrentAngle();
	}
	else {
		if (FControlRegimen == nmPropio) {
...
void TValvula4T::setCylinder(shared_ptr<TCylinder> cyl)
{
	NewCyl = cyl;
	NewCylConnected = true;
}
