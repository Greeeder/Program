...
void TValvulaContr::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	try {
		int Ctrl;
		...
		std::string Mode = node_controlled.attribute("ControlMode").value();
		if(Mode == "Angle") {
			FTipoContr = nmContrAngulo;
		} else if(Mode == "Fuel") {
			FTipoContr = nmContrFuel;
