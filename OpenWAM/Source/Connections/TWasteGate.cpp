...
void TWasteGate::LeeDatosInicialesXML(xml_node node_valve, int norden, bool HayMotor, TBloqueMotor *Engine) {
	try {
		int modo;
...
//FILE *fich=fopen(FileSALIDA,"a");
		...
		if(FGraficasWG) {
			if(FGrafLev) {
				Label = "\t" + PutLabel(5014) + "/" + std::to_string(lam) + "/" + PutLabel(4030) + PutLabel(902);
