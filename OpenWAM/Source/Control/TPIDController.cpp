...
void TPIDController::CabeceraResultadosMedControlador(stringstream& medoutput) {
	try {
		...
		if(FResMediosCtrl.Output) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3051) + PutLabel(901) + "/" + PutLabel(3057);
			medoutput << Label.c_str();
		}
		if(FResMediosCtrl.Error) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3053) + PutLabel(901);
			medoutput << Label.c_str();
		}
		if(FResMediosCtrl.POutput) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3054) + PutLabel(901);
			medoutput << Label.c_str();
		}
		if(FResMediosCtrl.IOutput) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3055) + PutLabel(901);
			medoutput << Label.c_str();
		}
		if(FResMediosCtrl.DOutput) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3056) + PutLabel(901);
			medoutput << Label.c_str();
		}
		if(FResMediosCtrl.Output_filt) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3051) + PutLabel(901) + "/" + PutLabel(3058);
			medoutput << Label.c_str();
		}
