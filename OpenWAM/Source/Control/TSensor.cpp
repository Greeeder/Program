...
void TSensor::ReadSensorXML(xml_node node_sensor) {
	...
	std::string Object = node_sensor.attribute("ObjectSensed").value();
	std::string Parameter = node_sensor.attribute("Parameter").value();
	...
	if(Object == "Simulation") {
		...
		if(Parameter == "Time") {
			...
			std::cout << "ERROR: Parametro " << Parameter << "en el objeto " << Object << "no valido, sensor: " << FNumeroSensor <<
					  std::endl;
		}
	} else if(Object == "Pipe") {
		...
		std::cout << "ERROR: Objeto " << Object << "no valido, sensor: " << FNumeroSensor << std::endl;
	}
...
void TSensor::CabeceraResultadosMedSensor(stringstream& medoutput) {
	try {
		...
		if(FResMediosSensor.Output) {
			Label = "\t" + PutLabel(5012) + "/" + std::to_string(FNumeroSensor) + "/" + PutLabel(3051) + PutLabel(901);
			medoutput << Label.c_str();
		}
		if(FResMediosSensor.Input) {
			Label = "\t" + PutLabel(5012) + "/" + std::to_string(FNumeroSensor) + "/" + PutLabel(3052) + PutLabel(901);
			medoutput << Label.c_str();
		}
