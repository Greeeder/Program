...
void TTable1D::LeeControllerXML(xml_node node_ctrl) {
	...
	if(FromFile) {
		...
		// fscanf(fich,"%d ",&fDimensiones);
		for(xml_node node_pt = GetNodeChild(node_table, "Bot_Turbocharger"); node_pt;
			node_pt = node_pt.next_sibling("Bot_Turbocharger")) {
	...
	std::string Type = node_table.attribute("Type").value();
...
void TTable1D::CabeceraResultadosMedControlador(stringstream& medoutput) {
	try {
		...
		if(FResMediosCtrl.Output) {
			Label = "\t" + PutLabel(5013) + "/" + std::to_string(fID) + "/" + PutLabel(3051) + PutLabel(901);
