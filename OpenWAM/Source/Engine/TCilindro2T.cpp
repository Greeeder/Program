		...
		if(FMotor->getCombustible() == nmMEP || FMotor->getCombustible() == nmMEC) {
			...
			if(FAnguloActual >= 90. && FAnguloActual <= 180.) {
				...
				Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) * pow(__units::degCToK(FTemperature),
						-0.53) * pow(FMotor->getGeometria().Diametro, -0.2)
					 * pow((6.18 * FCm + 0.417 * FCu) * FMotor->getAjusteTranCalEsc() + Fc2 * deltaP *
						   FMotor->getGeometria().CilindradaUnitaria / FMasaAtrapada / FRMezcla, 0.8);
			} else if(FAnguloActual >= 180. && FAnguloActual <= 270.) {
				// CARRERA DE ADMISION
				Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) * pow(__units::degCToK(FTemperature),
						-0.53) * pow(FMotor->getGeometria().Diametro, -0.2)
					 * pow((6.18 * FCm + 0.417 * FCu) * FMotor->getAjusteTranCalAdm() + Fc2 * deltaP *
						   FMotor->getGeometria().CilindradaUnitaria / FMasaAtrapada / FRMezcla, 0.8);
			} else { // CARRERAS DE COMPRESION Y EXPANSION
				Fh = 1.2e-2 * pow(__units::BarToPa(FPressure), 0.8) * pow(__units::degCToK(FTemperature),
						-0.53) * pow(FMotor->getGeometria().Diametro, -0.2)
					 * pow((FMotor->getWoschni().cw1 * FCm + FMotor->getWoschni().cw2 * FCu) + Fc2 * deltaP *
						   FMotor->getGeometria().CilindradaUnitaria / FMasaAtrapada / FRMezcla, 0.8);
			}
		}
