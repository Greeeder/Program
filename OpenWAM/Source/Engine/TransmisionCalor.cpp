...
void TransmisionCalor::Inicializar(stInicializar Inicializar)
{
...
void TransmisionCalor::crear_nodos_piston()
{
	...
	for (cont0 = 1; cont0 <= 10; cont0++) {
		...
		_NODOS_PIS.at(cont0).A_S = __cons::Pi * (pow((dp / 2 - _NODOS_PIS.at(cont0).X_II), 2) - pow((dp / 2 - _NODOS_PIS.at(cont0).X_SD), 2));
		_NODOS_PIS.at(cont0).A_I = __cons::Pi * (pow((dp / 2 - _NODOS_PIS.at(cont0).X_II + Aux), 2) - pow((dp / 2 - _NODOS_PIS.at(cont0).X_SD), 2));
		_NODOS_PIS.at(cont0).A_LI = 2 * __cons::Pi * _NODOS_PIS.at(cont0).ALTURA * (dp / 2 - _NODOS_PIS.at(cont0).X_II);
		if (((cont0 == 6) | (cont0 == 8))) {
			_NODOS_PIS.at(cont0).A_LD = 0;
			_NODOS_PIS.at(cont0).Area = _NODOS_PIS.at(cont0).ANCHURA * _NODOS_PIS.at(cont0).ALTURA / 2;
		}
		else {
			_NODOS_PIS.at(cont0).A_LD = 2 * __cons::Pi * _NODOS_PIS.at(cont0).ALTURA * (dp / 2 - _NODOS_PIS.at(cont0).X_SD);
...
void TransmisionCalor::crear_nodos_culata()
{
	...
	Ytotal = c_val / 2;
	//inyector
	Siny = __cons::Pi / 4 * pow((DINY / 2), 2);
	xiny = 4. / 3. * (Raiz2) / __cons::Pi  * (DINY / 2) * cos(__cons::Pi / 4);
	yiny = 4. / 3. * (Raiz2) / __cons::Pi  * (DINY / 2) * sin(__cons::Pi / 4);
	//valvula de escape
	SVE = __cons::Pi / 4 * pow((DVE / 2), 2);
	xVE = (Raiz2 * c_val - 4. / 3. * (Raiz2) / __cons::Pi  * (DVE / 2)) * cos(__cons::Pi / 4);
	yVE = (Raiz2 * c_val - 4. / 3. * (Raiz2) / __cons::Pi * (DVE / 2)) * sin(__cons::Pi / 4);
	//valvula de admision
	SVA = __cons::Pi / 4 * pow((DVA / 2), 2);
	xVA = (Raiz2 * c_val - 4. / 3. * (Raiz2) / __cons::Pi  * (DVA / 2)) * cos(__cons::Pi / 4);
	yVA = (Raiz2 * c_val - 4. / 3. * (Raiz2) / __cons::Pi  * (DVA / 2)) * sin(__cons::Pi / 4);
	...
	ST1 = L_per * L_per / 2;
	ST2 = c_val * c_val / 2;
	SC3E = (__cons::Pi / 2 + __cons::Pi / 4) * 1. / 2. * pow((DVE / 2), 2);
	SC3A = (__cons::Pi / 2 + __cons::Pi / 4) * 1. / 2. * pow((DVA / 2), 2);
	SEE = ST1 - ST2 - SC3E;
	SAA = ST1 - ST2 - SC3A;

	xT1 = 2. / 3. * L_per;
	xT2 = 2. / 3. * c_val;
	xC3E = c_val + 4. / 3. * sin(3 * __cons::Pi / 4) / (3 * __cons::Pi / 4) * (DVE / 2) * sin((__cons::Pi / 2 + __cons::Pi / 4) / 2);
	xC3A = c_val + 4. / 3. * sin(3 * __cons::Pi / 4) / (3 * __cons::Pi / 4) * (DVA / 2) * sin((__cons::Pi / 2 + __cons::Pi / 4) / 2);
	...
	yC3E = c_val - 4. / 3. * sin(3 * __cons::Pi / 4) / (3 * __cons::Pi / 4) * (DVE / 2) * cos((__cons::Pi / 2 + __cons::Pi / 4) / 2);
	yC3A = c_val - 4. / 3. * sin(3 * __cons::Pi / 4) / (3 * __cons::Pi / 4) * (DVA / 2) * cos((__cons::Pi / 2 + __cons::Pi / 4) / 2);
	...
	if (_Inicializar.ConfMotor_CULATA_A_PIPA_ADM == 0) {
		SP1_A = __cons::Pi * DVA * (dp / 2 - c_val);
		SP2_A = __cons::Pi * DVA * (dp / 2 - c_val);
		SP_A = SP1_A + SP2_A;
	}
	else {
	...
	if (_Inicializar.ConfMotor_CULATA_A_PIPA_ESC == 0) {
		SP1_E = __cons::Pi * DVE * (dp / 2 - c_val);
		SP2_E = __cons::Pi * DVE * (dp / 2 - c_val);
		SP_E = SP1_E + SP2_E;
	}
	else {
	...
	for (cont = 1; cont <= 4; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = SEE - 1. / 2. * (L_per * L_per - __cons::Pi / 4 * pot1);
	}

	for (cont = 5; cont <= 8; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = SAA - 1. / 2. * (L_per * L_per - __cons::Pi / 4 * pot1);
	}

	_NODOS_CUL.at(9).AREA_GAS = SCentral;
	double pot2 = pow((DINY / 2), 2);
	_NODOS_CUL.at(10).AREA_GAS = __cons::Pi * pot2;

	//'valvulas ADMISION
	double pot3 = pow((DVA / 2), 2);
	for (cont = 21; cont <= 22; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = __cons::Pi * pot3;
	}

	//'valvulas Escape
	double pot4 = pow((DVE / 2), 2);
	for (cont = 23; cont <= 24; cont++) {
		_NODOS_CUL.at(cont).AREA_GAS = __cons::Pi * pot4;
	}
	...
	for (cont = 21; cont <= 22; cont++) {
		_NODOS_CUL.at(cont).AREA_ADM = __cons::Pi / 4 * (pot3 - pot5);
	}


	//Vastago
	//For cont = 25 To 26
	//    _NODOS_CUL.at(cont).AREA_ADM = __cons::Pi * (F_VAST_ADM * DVA) * (dp - c_val + espesor_FD / 2 + DVA / 2)
	//Next cont
	for (cont = 25; cont <= 26; cont++) {
		_NODOS_CUL.at(cont).AREA_ADM = __cons::Pi * (F_VAST_ADM * DVA) * (dp / 2 - c_val + espesor_FD / 2 - DVA / 2);
	}
	//Pipas
	...
	for (cont = 23; cont <= 24; cont++) {
		_NODOS_CUL.at(cont).AREA_ESC = __cons::Pi / 4 * (pot6 - pot7);
	}

	//Vastago
	//For cont = 27 To 28
	//    _NODOS_CUL.at(cont).AREA_ESC = __cons::Pi * (F_VAST_ESC * DVE) * (dp - c_val + espesor_FD / 2 + DVE / 2)
	//Next cont

	//Febrero 2012
	for (cont = 27; cont <= 28; cont++) {
		_NODOS_CUL.at(cont).AREA_ESC = __cons::Pi * (F_VAST_ESC * DVE) * (dp / 2 - c_val + espesor_FD / 2 - DVE / 2);
	}
	...
	//Admision
	//
	//_NODOS_CUL.at(34).AREA_COOL = L_per * L_per - 1. / 2. * (__cons::Pi * (DINY / 2) ^ 2) - 2 * (__cons::Pi * (DVA / 2) ^ 2)
	//'Escape
	//_NODOS_CUL.at(35).AREA_COOL = L_per * L_per - 1. / 2. * (__cons::Pi * (DINY / 2) ^ 2) - 2 * (__cons::Pi * (DVE / 2) ^ 2)

	//Febrero 2012
	//_NODOS_CUL.at(34).AREA_COOL = Lcul * Lcul + 2 * L_per * Espesor_zona1_2 + Lcul * Espesor_zona1_2 - 2 * __cons::Pi / 4 * DVA ^ 2 'Febrero 2012
	//Escape
	//_NODOS_CUL.at(35).AREA_COOL = Lcul * Lcul + 2 * L_per * Espesor_zona1_2 + Lcul * Espesor_zona1_2 - 2 * __cons::Pi / 4 * DVE ^ 2 'Febrero 2012
	...
	_NODOS_CUL.at(15).AREA_COOL = 0.5 * pot8 - __cons::Pi * pot9 / 8;
	_NODOS_CUL.at(16).AREA_COOL = (pot10 - pot11 - 3. / 4. * __cons::Pi * pot9) / 2;
	_NODOS_CUL.at(17).AREA_COOL = (pot10 - pot11 - 3. / 4. * __cons::Pi * pot9) / 2;
	_NODOS_CUL.at(18).AREA_COOL = 0.5 * pot8 - __cons::Pi * pot9 / 8;
	_NODOS_CUL.at(30).AREA_COOL = __cons::Pi * (DVA / 2 + espesor_pipas) * 2. / 3. * (dp / 2 - c_val - DVA / 2 - espesor_pipas);
	_NODOS_CUL.at(31).AREA_COOL = __cons::Pi * (DVA / 2 + espesor_pipas) * 2. / 3. * (dp / 2 - c_val - DVA / 2 - espesor_pipas);
	_NODOS_CUL.at(34).AREA_COOL = Lcul * L_per + 2 * (L_per * Espesor_zona1_2 - (L_per - c_val) * 2. / 3. * (dp / 2 - c_val - DVA / 2 - espesor_pipas)) + Lcul * Espesor_zona1_2 - 2. / 3. * (dp / 2 - c_val - DVA / 2 - espesor_pipas) * Lcul - 2 * __cons::Pi * pot13 + (L_per - c_val) * Lcul - __cons::Pi * pot9 + (Lcul - 2 * DVA) * 2. / 3. * (dp / 2 - c_val - DVA / 2 - espesor_pipas);

	//Escape 'Modificadas para nuevo conducto en la culata:
	_NODOS_CUL.at(11).AREA_COOL = 0.5 * pot8 - __cons::Pi * pot12 / 8;
	_NODOS_CUL.at(12).AREA_COOL = (pot10 - pot11 - 3. / 4. * __cons::Pi * pot12) / 2;
	_NODOS_CUL.at(13).AREA_COOL = (pot10 - pot11 - 3. / 4. * __cons::Pi * pot12) / 2;
	_NODOS_CUL.at(14).AREA_COOL = 0.5 * pot8 - __cons::Pi * pot12 / 8;
	_NODOS_CUL.at(32).AREA_COOL = __cons::Pi * (DVE / 2 + espesor_pipas) * 2. / 3. * (dp / 2 - c_val - DVE / 2 - espesor_pipas);
	_NODOS_CUL.at(33).AREA_COOL = __cons::Pi * (DVE / 2 + espesor_pipas) * 2. / 3. * (dp / 2 - c_val - DVE / 2 - espesor_pipas);
	_NODOS_CUL.at(35).AREA_COOL = Lcul * L_per + 2 * (L_per * Espesor_zona1_2 - (L_per - c_val) * 2. / 3. * (dp / 2 - c_val - DVE / 2 - espesor_pipas)) + Lcul * Espesor_zona1_2 - 2. / 3. * (dp / 2 - c_val - DVE / 2 - espesor_pipas) * Lcul - 2 * __cons::Pi * pot14 + (L_per - c_val) * Lcul - __cons::Pi * pot12 + (Lcul - 2 * DVE) * 2. / 3. * (dp / 2 - c_val - DVE / 2 - espesor_pipas);
...
void TransmisionCalor::calcular_conductancias_conductivas()
{
		...
		for (cont2 = 1; cont2 <= nradiales; cont2++) {
			for (cont3 = 1; cont3 <= ncircunf; cont3++) {
				if (cont2 == 1) {
					...
					Kseg = (_NODOS_CIL.at(nodo_cil_seg).ANG_FIN - _NODOS_CIL.at(nodo_cil_seg).ANG_INI) * __cons::Pi / 180 * 4 * acum / log((1 + _NODOS_CIL.at(nodo_cil_seg).D_MAX / _NODOS_CIL.at(nodo_cil_seg).D_MIN) / 2);
		...
		for (cont2 = cont1; cont2 <= ncilin; cont2++) {
			...
			if ((ax1 == ax2)) {
				if (((c1 == c2) & (abs(r2 - r1) == 1)))
				{
					...
					fi01 = (_NODOS_CIL.at(cont1).ANG_INI - _NODOS_CIL.at(cont1).ANG_FIN) * __cons::Pi / 180;
					//cdg de los nodos
				...
				if ((r1 == r2)) {
					...
					if (((resta == 1) | ((ncircunf - resta) == 1))) {
						...
						fi01 = abs((_NODOS_CIL.at(cont1).ANG_INI - _NODOS_CIL.at(cont1).ANG_FIN) * __cons::Pi / 180);
						...
						//27/01/2014
						//fi02 = ((_NODOS_CIL.at(cont1).ANG_INI + _NODOS_CIL.at(cont1).ANG_FIN) / 2) - ((_NODOS_CIL.at(cont2).ANG_INI + _NODOS_CIL.at(cont2).ANG_FIN) / 2) * __cons::Pi / 180
				...
				if (((c1 == c2) & (r1 == r2) & (abs(ax1 - ax2) == 1))) {
					...
					fi01 = _NODOS_CIL.at(cont1).ANG_INI * __cons::Pi / 180;
					fi02 = _NODOS_CIL.at(cont1).ANG_FIN * __cons::Pi / 180;
	...
	//C_H_ASIENTO
	//
	//A_seat_adm = __cons::Pi * DVA * (espesor_FD / 2) / Sin(C_ANG_ASIENTO * __cons::Pi / 180) 'se asume altura de asiento de la válvula como la mitad del firedeck
	//A_seat_esc = __cons::Pi * DVE * (espesor_FD / 2) / Sin(C_ANG_ASIENTO * __cons::Pi / 180)
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	A_seat_adm = (__cons::Pi * espesor_FD / (2 * sin(2 * C_ANG_ASIENTO * __cons::Pi / 180))) * (2 * DVA * cos(C_ANG_ASIENTO * __cons::Pi / 180) - espesor_FD * sin(C_ANG_ASIENTO * __cons::Pi / 180));
	//se asume altura de asiento de la válvula como la mitad del firedeck
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto C_ANG_ASIENTO. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	A_seat_esc = (__cons::Pi * espesor_FD / (2 * sin(2 * C_ANG_ASIENTO * __cons::Pi / 180))) * (2 * DVE * cos(C_ANG_ASIENTO * __cons::Pi / 180) - espesor_FD * sin(C_ANG_ASIENTO * __cons::Pi / 180));
	...
	nodo2 = 23;
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (__cons::Pi / 2 + __cons::Pi / 4) * (DVE / 2) * espesor_FD / 2 'de pablo
	Area = (__cons::Pi / 2 + __cons::Pi / 4) / (2 * __cons::Pi) * A_seat_esc;
	k = -Kseat * Area;
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (pow(L_per, 2)) / 2 - (pow(c_val, 2)) / 2 - 3 * __cons::Pi / 8 * pow((DVE / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	nodo2 = 23;
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (__cons::Pi / 2 + __cons::Pi / 4) * (DVE / 2) * espesor_FD / 2
	Area = (__cons::Pi / 2 + __cons::Pi / 4) / (2 * __cons::Pi) * A_seat_esc;
	k = -Kseat * Area;
	...
	nodo2 = 22;
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (__cons::Pi / 2 + __cons::Pi / 4) * (DVA / 2) * espesor_FD / 2
	Area = (__cons::Pi / 2 + __cons::Pi / 4) / (2 * __cons::Pi) * A_seat_adm;
	k = -Kseat * Area;
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (pow(L_per, 2)) / 2 - (pow(c_val, 2)) / 2 - 3 * __cons::Pi / 8 * pow((DVA / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = pow((2 * c_val), 2) - 1. / 2. * __cons::Pi * (pow((DVA / 2), 2) + pow((DVE / 2), 2)) - __cons::Pi * pow((DINY / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DINY. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 2 * __cons::Pi * (DINY / 2) * espesor_FD / 2;
	//=2*__cons::Pi()*($B$3/2)*$B$4/2
	k = -k_cul2 * Area / dist;
	...
	nodo2 = 22;
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = __cons::Pi / 2 * (DVA / 2) * espesor_FD / 2
	Area = (__cons::Pi / 2) / (2 * __cons::Pi) * A_seat_adm;
	k = -Kseat * Area;
	...
	nodo2 = 24;
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = __cons::Pi / 2 * (DVE / 2) * espesor_FD / 2
	Area = (__cons::Pi / 2) / (2 * __cons::Pi) * A_seat_esc;
	k = -Kseat * Area;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 3 / 2 * __cons::Pi / 4 * (pow((DVE / 2 + espesor_pipas), 2) - pow((DVE / 2), 2));
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 3 / 2 * __cons::Pi / 4 * (pow((DVA / 2 + espesor_pipas), 2) - pow((DVA / 2), 2));
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi / 4 * (pow((DVA / 2 + espesor_pipas), 2) - pow((DVA / 2), 2));
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi / 4 * (pow((DVE / 2 + espesor_pipas), 2) - pow((DVE / 2), 2));
	k = -k_cul2 * Area / dist;
	...
	//nodo2 = 34
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (2 * c_val) ^ 2 - 2 * 1 / 4 * __cons::Pi * ((DVA / 2) ^ 2 + (DVE / 2) ^ 2) - __cons::Pi * (DINY / 2) ^ 2
	//k = -k_cul2 * Area / dist
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (2 * c_val * c_val) - 1. / 2. * __cons::Pi * (pow((DVA / 2 + espesor_pipas), 2)) - 1. / 2. * __cons::Pi * pow((DINY / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (2 * c_val * c_val) - 1. / 2. * __cons::Pi * (pow((DVE / 2 + espesor_pipas), 2)) - 1. / 2. * __cons::Pi * pow((DINY / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi * pow((F_VAST_ADM * DVA / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi * pow((F_VAST_ESC * DVE / 2), 2);
	k = -k_cul2 * Area / dist;
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi * F_VAST_ADM * DVA * espesor_pipas;
	k = -k_cul2 * Area / dist;
	...
	//nodo2 = 34
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (dp - (dp - c_val) - (DVA / 2) - espesor_pipas) * (2 * __cons::Pi * (F_VAST_ADM * DVA / 2))
	//k = -k_cul2 * Area / dist
	//Set reg_cond_cond = New CConductanciaConductiva
	//reg_cond_cond.ID_NODO1 = _NODOS_CUL.at(nodo1).ID_NODO
	//reg_cond_cond.ID_NODO2 = _NODOS_CUL.at(nodo2).ID_NODO
	...
	dist = distancia_nodos_CH(nodo1, nodo2);
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = __cons::Pi * F_VAST_ESC * DVE * espesor_pipas;
	k = -k_cul2 * Area / dist;
	...
	//nodo2 = 35
	//dist = distancia_nodos_CH(nodo1, nodo2)
	//Area = (dp - (dp - c_val) - (DVE / 2) - espesor_pipas) * (2 * __cons::Pi * (F_VAST_ESC * DVE / 2))
	//k = -k_cul2 * Area / dist
	//Set reg_cond_cond = New CConductanciaConductiva
	//reg_cond_cond.ID_NODO1 = _NODOS_CUL.at(nodo1).ID_NODO
	//reg_cond_cond.ID_NODO2 = _NODOS_CUL.at(nodo2).ID_NODO
	...
	nodo2 = 34;
	dist = distancia_nodos_CH(nodo1, nodo2);
	//Area = 1. / 2. * __cons::Pi * DINY * Espesor_zona1_2
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DINY. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 1. / 2. * __cons::Pi * DINY * (dp / 2 - c_val - DVA / 2);
	k = -k_cul2 * Area / dist;
	...
	nodo2 = 34;
	dist = distancia_nodos_CH(nodo1, nodo2);
	//Area = __cons::Pi * (DVA + 2 * espesor_pipas) * (2 * (dp - c_val) + espesor_pipas)
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 2 * __cons::Pi * (DVA + 2 * espesor_pipas) * (dp / 2 - c_val - DVA / 2);
	k = -k_cul2 * Area / dist;
	...
	nodo2 = 35;
	dist = distancia_nodos_CH(nodo1, nodo2);
	//Area = __cons::Pi * (DVE + 2 * espesor_pipas) * (2 * (dp - c_val) + espesor_pipas)
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = 2 * __cons::Pi * (DVE + 2 * espesor_pipas) * (dp / 2 - c_val - DVE / 2);
	k = -k_cul2 * Area / dist;
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVA. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (pow((L_cul / 2), 2) - pow(c_val, 2) - 3. / 4. * __cons::Pi * pow((DVA / 2 + espesor_pipas), 2)) / 2 - (0.5 * pow((L_per - c_val), 2) - __cons::Pi * pow((DVA / 2 + espesor_pipas), 2) / 8);
	...
	//    dist = distancia_nodos_CH(nodo1, nodo2)
	//    Area = ((L_cul / 2) ^ 2) - (c_val ^ 2) - (3. / 4. * __cons::Pi / 4 * (DVA ^ 2)) / 2 'febrero 2012'
	//    k = -k_cul2 * Area / dist
	//    Set reg_cond_cond = New CConductanciaConductiva
	//    reg_cond_cond.ID_NODO1 = _NODOS_CUL.at(nodo1).ID_NODO
	//    reg_cond_cond.ID_NODO2 = _NODOS_CUL.at(nodo2).ID_NODO
	...
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto DVE. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	//UPGRADE_WARNING: No se puede resolver la propiedad predeterminada del objeto c_val. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	Area = (pow((L_cul / 2), 2) - pow(c_val, 2) - 3. / 4. * __cons::Pi * pow((DVE / 2 + espesor_pipas), 2)) / 2 - (0.5 * pow((L_per - c_val), 2) - __cons::Pi * pow((DVE / 2 + espesor_pipas), 2) / 8);
	k = -k_cul2 * Area / dist;
	...
	//    dist = distancia_nodos_CH(nodo1, nodo2)
	//    Area = ((L_cul / 2) ^ 2) - (c_val ^ 2) - (3. / 4. * __cons::Pi / 4 * (DVE ^ 2)) / 2 'febrero 2012'
	//    k = -k_cul2 * Area / dist
	//    Set reg_cond_cond = New CConductanciaConductiva
	//    reg_cond_cond.ID_NODO1 = _NODOS_CUL.at(nodo1).ID_NODO
	//    reg_cond_cond.ID_NODO2 = _NODOS_CUL.at(nodo2).ID_NODO
...
void TransmisionCalor::CalcularTemperaturas(short Z, double *TCUL, double *TCULMAT, double *TCULVALV, double *TPIS, double *TCIL, double *TCULMAX, double *TCULMIN, double *TPISMAX, double *TPISMIN, double *TCILMAX, double *TCILMIN, double *TGASM)
{
	...
	else
	{
		...
		double A_h = (pot1 - pot2) * __cons::Pi / ncircunf;
		...
		for (cont2 = 1; cont2 <= ncilin; cont2++)
		{
			...
			{
				areac = abs(((_NODOS_CIL.at(cont2).ANG_FIN - _NODOS_CIL.at(cont2).ANG_INI) / 360 * 2 * __cons::Pi) * (_NODOS_CIL.at(cont2).D_MAX / 2) * (_NODOS_CIL.at(cont2).L_INF - _NODOS_CIL.at(cont2).L_SUP));
				K_fluidos_cil.at(cont2).at(5) = -_Inicializar.ConfMotor_BASE_H_CIL * areac;
			}
	...
	else
	{
		...
		K_fluidos_pis.at(3).at(4) = -hmean_oil * (pow(__cons::Pi, 2)) * 2 * Dgal * _NODOS_PIS.at(3).ANCHURA;
...
void TransmisionCalor::calculo_vcil(double Lcil, std::vector< std::vector<double> > * area_inst_nodos_cil)
{
	...
	//				area_inst_nodos_cil_aux.at(cont1).at(cont2) = (_NODOS_CIL.at(cont2).L_SUP - _NODOS_CIL.at(cont2).L_INF) * _NODOS_CIL.at(cont2).D_MIN * __cons::Pi * (_NODOS_CIL.at(cont2).ANG_FIN - _NODOS_CIL.at(cont2).ANG_INI) / 360;
	//			}
	//			else {
	//				area_inst_nodos_cil_aux.at(cont1).at(cont2) = (var_inst_nodal.at(cont1).hi - Lcil + _NODOS_CIL.at(cont2).L_SUP) * _NODOS_CIL.at(cont2).D_MIN * __cons::Pi * (_NODOS_CIL.at(cont2).ANG_FIN - _NODOS_CIL.at(cont2).ANG_INI) / 360;
	//			}
...
void TransmisionCalor::calculo_vcil_avanzado(double Lcil, std::vector< std::vector<double> > * area_inst_nodos_cil, std::vector< std::vector<std::string> > * matriz_geom_cil, short naxiales, short nradiales, short ncircunf)
{
	...
	for (cont1 = 1; cont1 <= n; cont1++) {
		for (int i = 0; i < matriz_geom_cil->size(); i++)
		{
			...
			//	area_inst_nodos_cil_aux.at(cont1).at(cont2) = (L_SUP - L_INF) * D_MIN * __cons::Pi * (ANG_FIN - ANG_INI) / 360;
			//}
			//else {
			//	area_inst_nodos_cil_aux.at(cont1).at(cont2) = (var_inst_nodal.at(cont1).hi - Lcil + L_SUP) * D_MIN * __cons::Pi * (ANG_FIN - ANG_INI) / 360;
			//}
...
void TransmisionCalor::Calcula_Calor_Woschni(double T, double p)
{
	...
	try
	{
		...
		if (_InicializarPorAngulo.ciclo_cerrado) {
			...
			if (_Inicializar.ConfMotor_MOTOR_TIPO_ENCENDIDO == "MEC") {
				cu_prima = pow((_Inicializar.ConfMotor_BASE_D / _Inicializar.ConfMotor_PISTON_DB), 2) * (_Inicializar.ConfMotor_PISTON_DB / 2) * (2 * __cons::Pi * _Inicializar.VarMed_N * _Inicializar.ConfMotor_CULATA_CTM);
	...
	{
		std::cout << "ERROR: Ajustes::Calcula_Calor_Woschni" << e.what();
		throw e;
	}
	catch (...)
	{
		std::cout << "ERROR: Ajustes::Calcula_Calor_Woschni" << " Error Desconocido";
		throw;
	}
}
