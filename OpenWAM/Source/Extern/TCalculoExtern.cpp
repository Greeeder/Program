...
void TCalculoExtern::LlamadaECU(double DeltaT, TBloqueMotor **Engine) {
	try {
		...
		double Cp_ent_turbina, R_ent_turbina, Gamma, Temp0Entrada, Presion0Entrada, GastoCorregido, RegimenTurboCorregido,
			   RelacionExpansion, MasaPorAdmision;
		int nparametros = 0;
		...
		FInputs[0] = FSensorTubo[0].valact; // "Gasto masico por la admision tras el compresor (kg/s)"
		FInputs[1] = FSensorTubo[1].valact;     // "Presion de admision (bar)"
		FInputs[2] = FSensorTubo[2].valact;  // "Temperatura de admision (degC)"
		FInputs[3] = FSensorTubo[3].valact; // "Gasto masico por la admision tras el intercooler (kg/s)"
		FInputs[4] = FSensorTubo[4].valact;     // "Gasto masico de EGR (kg/s)"
		FInputs[5] = FSensorTubo[5].valact;     // "Presion entrada DPF [bar]"
		FInputs[6] = FSensorTubo[6].valact;     // "Velocidad entrada DPF (m/s)"
		FInputs[7] = FSensorTubo[7].valact;  // "Temperatura entrada DPF (degC)"
		FInputs[8] = FSensorTubo[8].valact; // "Presion entrada de turbina (bar)"
		FInputs[9] = FSensorTubo[9].valact; // "Velocidad entrada de turbina (m/s)"
		FInputs[10] = FSensorTubo[10].valact; // "Temperatura entrada de turbina (ºC)"
		FInputs[11] = FSensorTubo[11].valact; // "Gasto entrada de turbina (kg/s)"
		FInputs[12] = FSensorTubo[12].valact; // "Fraccion masica O2 entrada de turbina (-)"
		FInputs[13] = FSensorTubo[13].valact; // "Fraccion masica CO2 entrada de turbina (-)"
		FInputs[14] = FSensorTubo[14].valact; // "Fraccion masica H2O entrada de turbina (-)"
		FInputs[15] = FSensorTubo[15].valact; // "Presion salida de turbina (bar)"
		FInputs[16] = FSensorTubo[16].valact; // "Velocidad salida de turbina (m/s)"
		FInputs[17] = FSensorTubo[17].valact; // "Temperatura salida de turbina (ºC)"
		FInputs[18] = FSensorTG[0].valact;    // "Regimen del turbogrupo (rpm)"
		FInputs[19] = FSensorTurbina[0].valact; // "Relacion cinematica de la turbina (-)"
		FInputs[20] = FSensorMotor[0].valact;   // "Posicion angular del motor"
		FInputs[21] = FSensorMotor[1].valact; // "Regimen de giro del motor (rpm)"
		FInputs[22] = FSensorMotor[2].valact; // "Instante de tiempo del motor (s)"

		Cp_ent_turbina = CalculoCompletoCpMezcla(FInputs[12], FInputs[13], FInputs[14], 0., __units::degCToK(FInputs[10]),
						 FCalculoGamma, nmMEP);
		R_ent_turbina = CalculoCompletoRMezcla(FInputs[12], FInputs[13], FInputs[14], 0, FCalculoGamma, nmMEP);
		Gamma = CalculoCompletoGamma(R_ent_turbina, Cp_ent_turbina, FCalculoGamma);

		Temp0Entrada = __units::degCToK(FInputs[10]) + pow(FInputs[9], 2.) / (2. * Cp_ent_turbina);
		Presion0Entrada = FInputs[8] * pow(Temp0Entrada / __units::degCToK(FInputs[10]), (Gamma / (Gamma - 1.)));

		GastoCorregido = FInputs[11] * pow(Temp0Entrada, 0.5) / Presion0Entrada / 0.1; // (kg*K^0.5/s/MPa)
		RegimenTurboCorregido = FInputs[18] / pow(Temp0Entrada, 0.5) / 60; // (rev/s/K^0.5)
		RelacionExpansion = Presion0Entrada / FInputs[15];

		MasaPorAdmision = FInputs[0] * 1e6 * 120 / (4 * FInputs[21]); // (mg/cc)
...
//	 }
		...
		// PARA EL CASO EN QUE SE UTILICE UNA BASE DE DATOS DE dFQL
		if(FInputs[22] > FTiempoPrevio && (FTiempo >= Ftmuestreoecu || FPrimeraVez)) {
			FTiempo = 0.;
			FPrimeraVez = false;
			// CALCULO DEL COMBUSTIBLE ACTUAL //
			FMasaFuel = FFuel->CalculaFuel(MasaPorAdmision, FInputs[21], FInputs[22]);
			FMasaFuelDeseado = FFuel->getFuelDeseado();

			// CALCULO DE LAS CONDICIONES DE INYECCION //
			FControlInyeccion->CalculaSistemaInyeccion(FMasaFuel, FInputs[21]);

			// DETERMINACION DEL TIPO DE CONTROL SOBRE TGV Y EGR //
			nmTipoControl TipoControl = FEGRV->DeterminacionTipoControl(FInputs[21], FMasaFuelDeseado);

			// CALCULO DE LA VALVULA DE EGR //
			FEGRV->CalculaEGRV(FMasaFuelDeseado, FInputs[21], MasaPorAdmision, FInputs[22]);

			// CALCULO DE LA TURBINA //
			FTGV[0]->CalculaTurbina(TipoControl, FMasaFuelDeseado, FInputs[21], FInputs[1], FInputs[19], RelacionExpansion,
									RegimenTurboCorregido, GastoCorregido, FInputs[22]);

			// DETERMINACION BARABA

		}
		FAjusteTransCalorCil->CalculaTCC(FInputs[22], FInputs[21]);

		//LEYES DE LIBERACION DE CALOR //

		//Asignacion del vector de parametros
		nparametros = Ffql->getNumParametros();
		FParametros = new double[nparametros];
		// Lo que sigue es funcion de los parametros que se desee usar en la interpolacion //
		FParametros[0] = MasaPorAdmision;  // Masa de aire (mg/cc)
		FParametros[1] = FMasaFuel * 1e6;     // Masa de combustible (en mg/cc)
		FParametros[2] = FInputs[21];       // Regimen (rpm)
		FParametros[3] = FInputs[1];        // Presion de admision (P2') (bar)
		FParametros[4] = FInputs[2];     // Temperatura de admision (T2') (degC)
		FParametros[5] = FControlInyeccion->GetSOI(
							 1); // SOI Se ha introducido un valor, pero en realidad se ha de calcular a partir del acceso a la ECU.
		if(MasaPorAdmision == 0) {
			FParametros[6] = 0.;
		} else {
			FParametros[6] = FMasaFuel * 1e6 / MasaPorAdmision; // Dosado absoluto
		}
		FParametros[7] = FInputs[4] * 1e6 * 120 / (4 * FInputs[21]); // (mg/cc);        // Masa de EGR (mg/cc)
		FParametros[8] = (FInputs[1] * 100000) / (287 * __units::degCToK(FInputs[2])); //Densidad en la admision.

		// Asignacion de actuadores en el orden preestablecido //
		int compo = 0;
		/*if(FInputs[22]<FTiempoPrevio){
		 FOutputs[0]=FCDEGRIni;
		 FOutputs[1]=FCDEGRIni;
		 FOutputs[2]=0.;
		 }else{
		 FOutputs[0]=FEGRV->getCDEntrante();
		...
		for(int i = 0; i < Engine[0]->getGeometria().NCilin; i++) {
			if(FInputs[22] < FTiempoPrevio) {
				FMasaFuel = FFuelIni * 1e-6; //3.324E-06;
				FOutputs[compo] = FFuelIni * 1e-6; //3.324E-06;
			} else {
				FOutputs[compo] = FMasaFuel;
			}
			compo++;
		}
		...
		for(int i = 0; i < Engine[0]->getGeometria().NCilin; i++) {
			if(FInputs[22] < FTiempoPrevio) {
				FOutputs[compo] = 2;
				FOutputs[compo + 1] = 900;
				int contador = 0;
				for(int j = 0; j < 8; j++) {
					FOutputs[compo + 2 + j + contador] = 0.;
					FOutputs[compo + 3 + j + contador] = 0.;
					contador++;
				}
			} else {
				FOutputs[compo] = FControlInyeccion->getNumeroInyecciones();
				FOutputs[compo + 1] = FControlInyeccion->getPresionInyeccion();
				int contador = 0;
				for(int j = 0; j < 8; j++) {
					FOutputs[compo + 2 + j + contador] = FControlInyeccion->GetSOI(j);
					FOutputs[compo + 3 + j + contador] = FControlInyeccion->GetMasaFuel(j);
					contador++;
				}
			}
			compo += 18;
		...
		if(FInputs[22] < FTiempoPrevio) {
			FOutputs[compo] = FCDEstatorIni;
			FOutputs[compo + 1] = FCDRotorIni;
			FOutputs[compo + 2] = FRendimientoIni;
		} else {
			FOutputs[compo] = FTGV[0]->getCDStator();
			FOutputs[compo + 1] = FTGV[0]->getCDRotor();
			FOutputs[compo + 2] = FTGV[0]->getRendimiento();
		}
		compo += 6; // Se suman 6 salidas, por si la turbina fuese twin
		FOutputs[compo] = FTGV[0]->getPosicionTurbina();
		compo += 1;

		// BARABAs
		FOutputs[compo] = FAjusteTransCalorCil->getCAdmision();
		FOutputs[compo + 1] = FAjusteTransCalorCil->getCEscape();
		compo += 2;

		//LEYES DE LIBERACION DE CALOR //
		for(int i = 0; i < Engine[0]->getGeometria().NCilin; ++i) {
			Ffql->calcula_angulos_combustion(FParametros, i);
			FOutputs[compo] = Ffql->calcula_fql(FParametros, FInputs[20], i);
			FOutputs[compo + 1] = Ffql->getInicio();
			;
			FOutputs[compo + 2] = Ffql->getFinal();
			;
			compo += 3;
		}
			...
			for(int i = 0; i <= nventuris - 1; ++i) {
				...
				for(int j = 0; j <= nsensventuri - 1; ++j) {
					...
					case nmPrGarVent:
						prent = Venturi[v]->getPressure();
						prgar = prent
								/ pow(1 + Venturi[v]->getgamma3() * pow(dynamic_cast<TCCDeposito *>
...
void TCalculoExtern::Lectura_Datos_Adicionales(const char *FileWAM, fpos_t &filepos) {
	try {
		double PosicionVGT = 0., CierreMaximo = 0.;
		double FuelMin = 0., FuelMax = 0.;
		int CicloCambioTC, CicloCambioTC2, *Ciclo, NumeroTramos, CicloEGRCerrado;
		double *CoefTCAdm, *CoefTCEsc;
		FILE *fich = fopen(FileWAM, "r");
		fsetpos(fich, &filepos);

		fscanf(fich, "%lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d ", &FTiempoPrevio, &FCDEGRIni, &CicloEGRCerrado,
			   &FCDEstatorIni, &FCDRotorIni, &FRendimientoIni, &FFuelIni, &FuelMin, &FuelMax,
			   &PosicionVGT, &CierreMaximo, &NumeroTramos);

		Ciclo = new int[NumeroTramos];
		CoefTCAdm = new double[NumeroTramos];
		CoefTCEsc = new double[NumeroTramos];

		for(int i = 0; i < NumeroTramos; i++) {
			fscanf(fich, "%d %lf %lf ", &Ciclo[i], &CoefTCAdm[i], &CoefTCEsc[i]);
		}

		Ffql->lee_leylib(FRutaTrabajo, fich);
		FFuel->LeeDatosEntrada(FRutaTrabajo, fich);
		FFuel->putFuelMin(FuelMin / 1e6);
		FFuel->putFuelMax(FuelMax / 1e6);
		FControlInyeccion->LeeDatosEntrada(FRutaTrabajo, fich);
		FEGRV->LeeDatosEntrada(FRutaTrabajo, fich);
		FEGRV->IniciaEGRV(FCDEGRIni, CicloEGRCerrado);
		FTGV[0]->LeeDatosEntrada(FRutaTrabajo, fich);
		FTGV[0]->PutPosicionTurbina(PosicionVGT);
		FTGV[0]->PutCierreMaximo(CierreMaximo);
		FAjusteTransCalorCil->IniciaTCC(NumeroTramos, Ciclo, CoefTCAdm, CoefTCEsc);
			...
			if(n1 == FSensorTubo[i].nodos - 1) {
				...
				case nmTempTubo:
					...
					FSensorTubo[i].valact = ((2 * FSensorTubo[i].ctetiempo - deltaT) * FSensorTubo[i].valant + deltaT *
											 FSensorTubo[i].ganancia * (FSensorTubo[i].valreal + FSensorTubo[i].valrealant))
		...
		for(int i = 0; i <= FNSensVenturis - 1; i++) {
			...
			case nmPrEntVent:
				...
				FSensorVent[i].valact = ((2 * FSensorVent[i].ctetiempo - deltaT) * FSensorVent[i].valant + deltaT *
										 FSensorVent[i].ganancia * (FSensorVent[i].valreal + FSensorVent[i].valrealant))
			...
			case nmPrGarVent:
				...
				prgar = prent
						/ pow(
							...
							* pow(dynamic_cast<TCCDeposito *>(Venturi[FSensorVent[i].venturi -
									1]->getCCLateral())->getValvula()->getCRecuperacion(), 2.),
		...
		for(int i = 0; i <= FNSensMotor - 1; i++) {
			...
			case nmAngulo:
				...
				FSensorMotor[i].valact =
					((2 * FSensorMotor[i].ctetiempo - deltaT) * FSensorMotor[i].valant + deltaT * FSensorMotor[i].ganancia *
