...
//#define usetry true
#define WriteINS true
#define ConcentricElement true
#define ParticulateFilter true
...
enum nmTurbineType {
	...
	nmVariableGeometry,
	/* !< The turbine discharge coefficients are modified externally. */
...
enum nmAxisSpeedCalculation {
	nmFixed = 0,
	/* !< The turbocharger speed is constant during all the dimulation */
...
enum nmControlMethod {
	nmCtlConstant = 0, nmCtlPID = 1, nmCtlTable = 2, nmCtlSwitch = 3, nmGainCtrl = 4, nmExtCtrl = 5
};
...
struct stResInstantTurbina {
	...
	bool PlenumPressure;
	double *PlenumPressureINS;
	bool VoluteOutletPressure;
	double *VoluteOutletPressureINS;
};
...
struct stResMediosDPF {
	...
	bool WallSootMass;
	double WallSootMassSUM;
	double WallSootMassMED;
	bool LayerSootMass;
	double LayerSootMassSUM;
	double LayerSootMassMED;

	bool LayerSootMassDep;
	double LayerSootMassDepSUM;
	double LayerSootMassDepMED;

	bool CVSootMass;
	double CVSootMassSUM;
	double CVSootMassMED;
	bool BeamSootMass;
	double BeamSootMassSUM;
	double BeamSootMassMED;
	bool DPFSootMass;
	double DPFSootMassSUM;
	double DPFSootMassMED;

	bool EspesorSootIn;
	double EspesorSootInSUM;
	double EspesorSootInMED;
	...
	bool KwallClean;
	double KwallCleanSUM;
	double KwallCleanMED;
	bool KwallLoaded;
	double KwallLoadedSUM;
	double KwallLoadedMED;
	bool Kwall;
	double KwallSUM;
	double KwallMED;
	...
	double Q2MED;
	bool ShapeFactor;
	double ShapeFactorSUM;
	double ShapeFactorMED;
	...
	bool EficienciaBrown;
	double EficienciaBrownSUM;
	double EficienciaBrownMED;
	bool EficienciaInter;
	double EficienciaInterSUM;
	double EficienciaInterMED;
	bool EficienciaIner;
	double EficienciaInerSUM;
	double EficienciaInerMED;
	bool EficienciaPL;
	double EficienciaPLSUM;
	double EficienciaPLMED;
};
...
struct stResInstantDPF {
	...
	double MasaSootINS;
	bool WallSootMass;
	double WallSootMassINS;
	bool LayerSootMass;
	double LayerSootMassINS;

	bool LayerSootMassDep;
	double LayerSootMassDepINS;

	bool CVSootMass;
	double CVSootMassINS;
	bool BeamSootMass;
	double BeamSootMassINS;
	bool DPFSootMass;
	double DPFSootMassINS;

	bool EspesorSootIn;
	double EspesorSootInINS;
	...
	bool KwallClean;
	double KwallCleanINS;
	bool KwallLoaded;
	double KwallLoadedINS;
	bool Kwall;
	double KwallINS;
	...
	bool ShapeFactor;
	double ShapeFactorINS;
	bool EficienciaBrown;
	double EficienciaBrownINS;
	bool EficienciaInter;
	double EficienciaInterINS;
	bool EficienciaIner;
	double EficienciaInerINS;
	bool EficienciaPL;
	double EficienciaPLINS;
};
...
inline double CalculoUfgasoil(double Temperature) {
	...
	Ufgasoil = -1234157.8 - 256.4 * __units::degCToK(Temperature) + 3.47686 * pow(__units::degCToK(Temperature),
			   2) - 0.00134905 * pow(__units::degCToK(Temperature), 3)
			   + 0.000000227565 * pow(__units::degCToK(Temperature), 4) - 1458487. / __units::degCToK(Temperature);
	return Ufgasoil;
}
;
...
inline void ImposeCompositionXML(xml_node node_comp, double* Comp, bool EGR, bool Fuel,
	...
	if(SpeciesModel == nmCalculoCompleto) {
		for(xml_node Specie = node_comp.child("Specie"); Specie; Specie = Specie.next_sibling("Specie")) {
...
inline int IDtoInt(std::string ID){
	int i;

	std::size_t found = ID.find('-');
	if (found != std::string::npos){
		i = stoi(ID.erase(0, found + 1));
	}
	else{
		i = stoi(ID);
	}
...
template<class T>
inline T* FindObjectByID(const std::vector<T *> objs, string ID) {
	int id_i = IDtoInt(ID);
	for (auto obj : objs) {
		if (obj->getID() == id_i) {
			return obj;
...
template<class T>
inline shared_ptr<T> FindObjectByID(const std::vector<shared_ptr<T>>& objs, string ID) {
	int id_i = IDtoInt(ID);
	for (auto obj : objs) {
		if (obj->getID() == id_i) {
			return obj;
...
/*!
 * \brief	Reads gas properties.
 *
 * \author	F.J. Arnau (farnau@mot.upv.es)
 * \date	17/05/2016
 *
 * \param	node	 	The XML node.
 * \param [in,out]	p	The pressure [Pa].
 * \param [in,out]	T	The temperature [K].
 * \param [in,out]	u	The gas velocity [m / s].
 * \param [in,out]	Y	The mass fraction array [-].
 * \param	fluid	 	The fluid componenents array.
 */
inline void ReadGasProperties(xml_node node, double& p, double& T, double& u, RowVector& Y, const TComponentArray_ptr& fluid){

	p = __units::BarToPa(GetXMLPressure(node, "Pressure"));
	T = __units::degCToK(GetXMLTemperature(node, "Temperature"));
	if (node.attribute("Velocity"))
		u = GetXMLSpeed(node, "Velocity");


	bool found = false;
	xml_node node_comp = GetNodeChild(node, "Composition");
	for (xml_node node_c = node_comp.child("Cmp_Component"); node_c; node_c = node_c.next_sibling()){
		int i = 0;
		found = false;
		while (!found && i < fluid->size()){
			if (fluid->at(i)->getName() == node_c.attribute("name").as_string()){
				Y(i) = GetAttributeAsDouble(node_c, "fraction");
				found = true;
			}
			i++;
		}
		if (!found){
			cout << "ERROR: Reading initial composition " << endl;
			cout << "       Chemical component " << node_c.attribute("name").as_string() << " is not defined in the working fluid" << endl;
		}
	}
	if (Y.size() == 1) {
		Y(0) = 1.;
 ...
 * \date 17/07/2016
 *
 * Reads gas properties from an ::xml_node. This properties are provided
 ...
 * \param [in,out] Y The mass fraction array. [-]
 * \param fluid The fluid componenents array.
 * \param [in,out] x The positions where the properties are known. [m]
 */
inline void ReadGasProperties(const xml_node& node, RowVector* p,
	RowVector* T, RowVector* u, RowVector* Y,
	const TComponentArray_ptr& fluid, RowVector* x){

	auto n_points = CountNodes(node, "GasProps_Point");
	std::string p_units = node.child("Units").attribute("Pressure").as_string();
	std::string T_units = node.child("Units").attribute("Temperature").as_string();
	std::string u_units = node.child("Units").attribute("Speed").as_string();
	std::string d_units = node.child("Units").attribute("Length").as_string();
	p->setZero(n_points);
	T->setZero(n_points);
	u->setZero(n_points);
	x->setZero(n_points);
	Uint i = 0;
	for (const auto& point: node.children("GasProps_Point")) {
		(*x)(i) = to_metres(point.attribute("Position").as_double(), d_units);
		(*p)(i) = __units::BarToPa(to_bar(point.attribute("Pressure").as_double(), p_units));
		(*T)(i) = __units::degCToK(to_celsius(point.attribute("Temperature").as_double(), T_units));
		if (point.attribute("Velocity")) {
			(*u)(i) = to_m_s(point.attribute("Velocity").as_double(), u_units);
		}
		i += 1;
	}
