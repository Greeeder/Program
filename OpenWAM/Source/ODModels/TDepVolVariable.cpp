...
void TDepVolVariable::LeeDatosDepVolVariable(const char *FileWAM, fpos_t &filepos, bool HayMotor) {
	try {
		...
		FVolumenMuerto = __geom::Cylinder_volume(FDiametro, FCarrera) / (FRelCompre - 1.);
...
void TDepVolVariable::LeeDatosDepVolVariableXML(xml_node node_plenum, bool HayMotor) {
	try {
		...
		const char_t *ControlRegimen = node_vblvol.attribute("SpeedControl").value();
		...
		if(FControlRegimen == nmPropio) {
			...
			FRelacionVelocidades = GetAttributeAsDouble(node_vblvol, "SpeedRatio");
		} else {
			std::cout << "ERROR: TDepVolVariable::LeeDatosIniciales Lectura del Control del Regimen erronea " << std::endl;
			throw Exception(" ");
		}
