...
void TDeposito::LeeDatosGeneralesDepositosXML(xml_node node_plenum) {
	try {
		...
		if(FNumeroEspecies == 4 || FNumeroEspecies == 10)
			HayFuel = true;
		ImposeCompositionXML(node_comp, FFraccionMasicaEspecie, FHayEGR, HayFuel, FCalculoEspecies);
		...
		if(FTipoDeposito != nmDepVolVble) {
			...
			FPresionIsen = pow(FPressure / FPresRef, __Gamma::G5(FGamma));
			FAsonido = sqrt(FGamma * FRMezcla * __units::degCToK(FTemperature)) / __cons::ARef;
			FMasa = FVolumen * FGamma * __units::BarToPa(FPressure) / pow(FAsonido * __cons::ARef, 2.);
			for(int j = 0; j < FNumeroEspecies - FIntEGR; j++) {
				FMasaEspecie[j] = FMasa * FFraccionMasicaEspecie[j];
			}
		} else {
...
void TDeposito::UpdateInitialData(double p, double T) {
	FPressure = p;
	FTemperature = T;
	FPresionIsen = pow(FPressure / FPresRef, __Gamma::G5(FGamma));
	FAsonido = sqrt(FGamma * FRMezcla * __units::degCToK(FTemperature)) / __cons::ARef;
...
void TDeposito::HeaderInstantaneousResultsDep(stringstream& insoutput, stEspecies *DatosEspecies) {
	try {
		...
		if(FResInstantDep.Pressure) {
			Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4006) + PutLabel(908);
			insoutput << Label.c_str();
		}
		if(FResInstantDep.Temperature) {
			Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4005) + PutLabel(910);
			insoutput << Label.c_str();
		}
		if(FResInstantDep.Volumen) {
			Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4003) + PutLabel(912);
			insoutput << Label.c_str();
		}
		if(FResInstantDep.Masa) {
			Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4004) + PutLabel(913);
			insoutput << Label.c_str();
		}
		if(FResInstantDep.FraccionMasicaEspecies) {
			for(int i = 0; i < FNumeroEspecies - FIntEGR; i++) {
				Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4023) + PutLabel(
							901) + "/" + DatosEspecies[i].Nombre;
				insoutput << Label.c_str();
			}
		}
		if(FResInstantDep.Gamma) {
			Label = "\t" + PutLabel(5003) + "/" + std::to_string(FNumeroDeposito) + "/" + PutLabel(4017) + PutLabel(901);
			insoutput << Label.c_str();
		}
