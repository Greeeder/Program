...
void TTurbina::LeeTurbina(const char *FileWAM, fpos_t &filepos) {
	try {
		...
		if(FTipoTurbina == nmTurbineMap) {
			...
			fscanf(fich, "%lf %lf %lf %lf ", &FDiametroRodeteOut, &FDiametroTuerca, &FDiametroTurbinaIn, &FBladeHeight);
			fscanf(fich, "%lf ", &FCriticalAngle);
			FCriticalAngle = __units::DegToRad(FCriticalAngle);
			fscanf(fich, "%lf ", &FBeta);
			FBeta = __units::DegToRad(FBeta);
			FMapa = new TTurbineMap();
			FMapa->LoadTurbineMap(fich, this);
			...
			if(FMapa->IsFixed()) {
				...
				if(vn == 1) {
					...
					FVaneless = false;
					fscanf(fich, "%d ", &FZ0);
					fscanf(fich, "%lf ", &VGTtoAlpha2);
				}
			} else {
				fscanf(fich, "%lf ", &VGTtoAlpha1);
				fscanf(fich, "%lf ", &VGTtoAlpha2);
				fscanf(fich, "%lf ", &FLTE);
				fscanf(fich, "%d ", &FZ0);
				fscanf(fich, "%lf ", &FR2geom);
			}
			...
			if(ac == 1) {
				...
				if(FTipoDeposito == nmTurbinaSimple) {
					...
					InID.resize(2);
					VolID.resize(2);
					fscanf(fich, "%d %d %d %d %d", &InID[0], &VolID[0], &InID[1], &VolID[1], &OutID);
				}
...
void TTurbina::LeeTurbinaXML(xml_node node_turb) {
	try {
		...
		if(FTipoTurbina == nmTurbineMap) {
			...
			FCriticalAngle = GetXMLAngle(node_turb, "CriticalAngle");
			FCriticalAngle = __units::DegToRad(FCriticalAngle);
			FBladeHeight = GetXMLLength(node_turb, "BladeHeight");
			FBeta = GetXMLAngle(node_turb, "Beta");
			FBeta = __units::DegToRad(FBeta);
			...
			if(FMapa->IsFixed()) {
				FVaneless = GetAttributeAsBool(node_turb, "Vaneless");
				if(!FVaneless) {
					FZ0 = GetAttributeAsInt(node_turb, "Z0");
					VGTtoAlpha2 = GetAttributeAsDouble(node_turb, "VGTtoAlpha2");
				}
			} else {
				VGTtoAlpha1 = GetAttributeAsDouble(node_turb, "VGTtoAlpha1");
				VGTtoAlpha2 = GetAttributeAsDouble(node_turb, "VGTtoAlpha2");
				FLTE = GetAttributeAsDouble(node_turb, "LTE");
				FR2geom = GetAttributeAsDouble(node_turb, "R2geom");
				FZ0 = GetAttributeAsInt(node_turb, "Z0");
			}

			int numctrl = CountNodes(node_turb, "Actuator");
			std::string ctrl;
			for(xml_node node_ctrl = GetNodeChild(node_turb, "Actuator"); node_ctrl;
			...
			if(FIsAcoustic) {
				iVector InID;
				InID.push_back(GetAttributeAsInt(node_turb, "InID1"));
				iVector VolID;
				VolID.push_back(GetAttributeAsInt(node_turb, "VolID1"));
				if(FTipoDeposito == nmTurbinaTwin) {
					InID.push_back(GetAttributeAsInt(node_turb, "InID2"));
					VolID.push_back(GetAttributeAsInt(node_turb, "VolID2"));
				}
...
double TTurbina::CalculateL2(double Position) {
	double alpha = VGTtoAlpha1 * Position + VGTtoAlpha2;
	double r2 = sqrt(pow2(FR2geom - FLTE * cos(alpha)) + pow2(FLTE * sin(alpha)));
	double gamma = __cons::Pi / FZ0;

	return 2 * FR2geom * sin(gamma) * cos(alpha);
}
