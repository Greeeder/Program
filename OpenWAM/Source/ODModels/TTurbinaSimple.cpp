	...
	try {
		...
		if(DeltaT > 0) {
			...
			if(!FIsAcoustic) {
				...
				FTempEntrada[0] = FAcTurb->T3(0);
				FPresionEntrada[0] = FAcTurb->P3(0);

				FGastoEntrada[0] = FAcTurb->MassIn(0);

				FTempSalida = FAcTurb->T4();

				FPresionSalida = FAcTurb->P4();

				FVelocidadEntrada[0] = FGastoEntrada[0] * FRMezcla * FTempEntrada[0] / (__units::BarToPa(
										   FPresionEntrada[0]) * FAcTurb->SIn(0));
			...
			double T_Red = FTempEntrada[0] + pow2(FVelocidadEntrada[0]) / (2. * cpte);
			double P_Red = FPresionEntrada[0] * pow(T_Red / FTempEntrada[0], (gam_te / (gam_te - 1.0)));
			...
			if(!FThereIsHTM) {
				...
				if(fabs(FGastoEntrada[0]) > 1e-10) {
					...
					//FTempEntrada[0] = FTempEntrada[0] - FHTM->Turb_Heat_Flow() / cpte / fabs(FGastoEntrada[0]);
					FTempEntrada[0] = FTempEntrada[0] - FHTM->TurbineHeatFlow() / cpte / fabs(FGastoEntrada[0]);

					//if(FTempEntrada[0] < FHTM->NodeTemperature(10) && FHTM->Turb_Heat_Flow() > 0) {
					if(FTempEntrada[0] < __Ambient::T_K && FHTM->TurbineHeatFlow() > 0) {

						FTempEntrada[0] = __Ambient::T_K;

						//} else if(FTempEntrada[0] > FHTM->NodeTemperature(3) && FHTM->Turb_Heat_Flow() < 0) {
					} else if(FTempEntrada[0] > FHTM->getTemperatureNode(0) && FHTM->TurbineHeatFlow() < 0) {

						FTempEntrada[0] = FHTM->getTemperatureNode(0);
			...
			FGastoCorregido[0] = FGastoEntrada[0] * sqrt(T_Red) / P_Red / 0.1;
			FRegimenCorregido[0] = FRegimen / sqrt(T_Red);
			...
			if(FTipoTurbina == nmTurbineMap) {
				...
				if(FMapa->IsExtrapolated()) {
					FMapa->InterpExtendedMap(__units::RPMToRPS(FRegimenCorregido[0]), FRelacionExpansion[0], FRelacionCinematica[0], FRack);
					FMapa->CorrectRotorEffArea((FTemperature + 273) / FTemp0Entrada[0]);
				} else {
					FMapa->CurrentEffectiveSection(__units::RPMToRPS(FRegimenCorregido[0]), FRelacionExpansion[0], FRack,
			...
			if((FGastoEntrada[0] > 0.) && (FEntalpia0Entrada[0] - FEntalpiaIsenSalida[0]) >= 0.) {
				...
				if(FCalRendTurbina == nmWatson) {
					if(FRelacionCinematica[0] <= 0 || FRelacionCinematica[0] >= 1.19) {
						...
						FRendTurbina[0] = 0.004022
										  + FRelacionCinematica[0] * (1.55766 + FRelacionCinematica[0] * (-0.511626 + FRelacionCinematica[0] *
...
void TTurbinaSimple::CabeceraResultadosMedTurb(stringstream& medoutput) {
	try {
		...
		if(FResMediosTurbina.Trabajo) {
			Label = "\t" + PutLabel(5009) + "/" + std::to_string(FNumeroTurbina) + "/" + PutLabel(4021) + PutLabel(907);
...
void TTurbinaSimple::LeeResultadosInstantTurbXML(xml_node node_trb) {
	...
	try {
		...
		for(xml_attribute parameter = node_ins.attribute("Parameter");
			...
			if(value == "Power") {
				...
				FResInstantTurbina.RelaCinematica = true;
			} else if(value == "CorrectedMassFlow") {
				FResInstantTurbina.GastoCorregido = true;
			} else if(value == "CorrectedSpeed") {
				FResInstantTurbina.RegimenCorregido = true;
			} else if(value == "ExpansionRatio") {
				FResInstantTurbina.RelacionExpansion = true;
			} else if(value == "VoluteOutletPressure") {
				FResInstantTurbina.VoluteOutletPressure = true;
			} else if(value == "PlenumPressure") {
...
void TTurbinaSimple::ImprimeResultadosInstantTurb(stringstream& insoutput) {
	try {
		...
		if(FResInstantTurbina.VoluteOutletPressure) {
			insoutput << "\t" << __units::PaToBar(FAcTurb->VoluteOutletp().mean());
			// [bar]
		}
		if(FResInstantTurbina.PlenumPressure) {
			insoutput << "\t" << FPressure;
