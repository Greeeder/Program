		...
		if(DeltaT > 0) {
			...
			for(int i = 0; i < 2; i++) {
				...
				if(!FThereIsHTM) {
					...
					FTempEntrada[i] = FTempEntrada[i] - FHTM->TurbineHeatFlow() / cpte[i] / AbsoluteMass;
					// comprobacion para pasos iniciales.
					if(FTempEntrada[i] < __Ambient::T_K)
						FTempEntrada[i] = __Ambient::T_K;

					FVelocidadEntrada[i] = FGastoEntrada[i] * FRMezcla * FTempEntrada[i] / (__units::BarToPa(
											   FPresionEntrada[i]) * FAcTurb->SIn(i));

					FTemp0Entrada[i] = FTempEntrada[i] + pow2(FVelocidadEntrada[i]) / (2. * cpte[i]);

				}

				FPresion0Entrada[i] = FPresionEntrada[i] * pow(FTemp0Entrada[i] / FTempEntrada[i], (cpte[i] / FRMezcla));
			...
			if(FTipoTurbina == nmTurbineMap) {
				...
				for(int i = 0; i < 2; i++) {
					if(FMapa->IsExtrapolated()) {
						FMapa->InterpExtendedMap(FRegimenCorregido[i] / 60., FRelacionExpansion[i], FRelacionCinematica[i], FRack);
					} else {
