...
TOutputResults::TOutputResults(string case_name){

	std::string Extension = ".xml";
	std::string Base = case_name;
	std::string::size_type i = Base.find(Extension);

	if (i != std::string::npos)
		Base.erase(i, Extension.length());

	PlotsFileName = Base + "_plots.dat";
	CycleOutFileName = Base + "_cycles.dat";
...
void TOutputResults::CreatePlotsHeader()
{
	HeaderForPlots.push_back("Time[s]");
	HeaderForPlots.push_back("DeltaTime[s]");
	for (auto obj : ObjectForPlots) {
		obj->HeaderForPlots(HeaderForPlots);
	}
	Plots.resize(HeaderForPlots.size());
}

void TOutputResults::CreateCycleOutHeader()
{

	HeaderForCycleOut.push_back("Time[s]");
	HeaderForCycleOut.push_back("Cycle[-]");
	for (auto obj : ObjectForCycleOut) {
		obj->HeaderForCycleOutput(HeaderForCycleOut);
	}
}

void TOutputResults::CycleOutToFile(std::string separator){

	std::ofstream file(CycleOutFileName);
	if (file.is_open()){
		for (auto head : HeaderForCycleOut){
			file << head << separator;
		}
		file << endl;
		auto cols = HeaderForCycleOut.size();
		auto rows = CycleOutput.size();

		for (int j = 0; j < rows; j++){
			for (int i = 0; i < cols; i++){
				file << CycleOutput.at(j).at(i) << separator;
			}
			file << endl;
		}
	}
...
void TOutputResults::IntegrateCycleOutput(double t, int ncycle)
{

	std::vector<float> out;
	out.push_back(t);
	out.push_back((float)ncycle);

	for (auto obj : ObjectForCycleOut) {
		obj->IntegrateOutput(Plots, out);
	}
	CycleOutput.push_back(out);

}

void TOutputResults::StorePlots(double t)
{
	if (t > next_t) {
		double dt = t - previous_t;

		Plots.at(0).push_back(t);
		Plots.at(1).push_back(dt);

		for (auto obj : ObjectForPlots) {
			obj->StoreOutput(Plots);
		}

		next_t += sampling_interval;
		previous_t = t;
	}
}

void TOutputResults::PlotsToFile(std::string separator){

	std::ofstream file(PlotsFileName);
	if (file.is_open()){
		for (auto head : HeaderForPlots){
			file << head << separator;
		}
		file << endl;
		auto cols = HeaderForPlots.size();
		auto rows = Plots.at(0).size();

		for (int j = 0; j < rows; j++){
			for (int i = 0; i < cols; i++){
				file << Plots.at(i).at(j) << separator;
			}
			file << endl;
		}
	}
...
void TOutputResults::InitIntegrableInstantaneousResults(
	const vector <Integrable_ptr> & integrables) {
	for (auto object: integrables) {
		if (object->getInsOutputStatus() == true) {
			InsIntegrable.push_back(object);
		}
	}
}
...
void TOutputResults::ReadAverageResultsXML(xml_node node_openwam, TTubo** Pipe, bool EngineBlock, TBloqueMotor** Engine,
		TDeposito **Plenum, TEjeTurbogrupo** Axis, TCompresor** Compressor,
		TTurbina** Turbine, TCondicionContorno** BC, TDPF** DPF, TCCCompresorVolumetrico** Root, TVenturi** Venturi,
		TSensor** Sensor, TController** Controller, int TotalCycles, char* ModelName) {

	if(node_openwam.child("OutputSetup")) {
		...
		std::string OutMode = node_outsetup.attribute("OutputMode").value();
		...
		for(xml_node node_pipe = GetNodeChild(node_blockpipes, "Bop_Pipe"); node_pipe;
		...
		for(xml_node node_cyl = GetNodeChild(node_engine, "Eng_Cylinder"); node_cyl;
		...
		for(xml_node node_plenum = GetNodeChild(node_blockplenum, "Bod_Plenum"); node_plenum;
		...
		for(xml_node node_tch = GetNodeChild(node_blockplenum, "Bot_Turbocharger"); node_tch;
		...
		for(xml_node node_cmp = GetNodeChild(node_blockcmp, "Boc_Compressor"); node_cmp;
		...
		for(xml_node node_con = GetNodeChild(node_blockcon, "Bob_Connection"); node_con;
			node_con = node_con.next_sibling("Bob_Connection")) {
		...
		for(xml_node node_sen = GetNodeChild(node_blocksen, "Bos:Sensor"); node_sen;
		...
		for(xml_node node_ctrl = GetNodeChild(node_blockctr, "Bct_Controller"); node_ctrl;
...
void TOutputResults::ReadInstantaneousResults(const char* FileWAM, fpos_t &filepos, TBloqueMotor** Engine,
		TDeposito** Plenum, TTubo** Pipe, TVenturi** Venturi, TCondicionContorno** BC, TDPF** DPF,
...
void TOutputResults::ReadInstantaneousResultsXML(xml_node node_openwam, TBloqueMotor** Engine, TDeposito** Plenum,
	...
	if(node_openwam.child("OutputSetup")) {
		...
		int TurboID;
		xml_node node_blocktch = GetNodeChild(node_openwam, "BlockOfTurbochargers");
		for(xml_node node_tch = GetNodeChild(node_blocktch, "Bot_Turbocharger"); node_tch;
			node_tch = node_tch.next_sibling("Bot_Turbocharger")) {
...
void TOutputResults::ReadSettings(xml_node node){

	auto node_set = GetNodeChild(node, "OutputSettings");
	sampling_interval = GetAttributeAsDouble(node_set, "SampleInterval");
	next_t = sampling_interval;
}
...
void TOutputResults::ReadSpaceTimeResultsXML(xml_node node_openwam, TTubo** Pipe, TBloqueMotor** Engine,
		TDeposito **Plenum) {
					...
					for(int k = 0; k < STPipe[j]->getNin(); ++k) {
						if(STPipe[j]->getFormulacionLeyes() == nmConArea) {
							...
							double massflow = __units::BarToPa(STPipe[j]->GetPresion(k)) / pow2(STPipe[j]->GetAsonido(
												  k) * __cons::ARef) * STPipe[j]->GetGamma(k) * STPipe[j]->GetVelocidad(k)
											  * __cons::ARef * __geom::Circle_area(STPipe[j]->GetDiametro(k));
...
void TOutputResults::HeaderInstantaneousResults(TCalculoExtern *EXTERN, bool ThereIsDLL, bool EngineBlock,
	...
	if(WriteInsHeader) {
		...
		// Integrable objects header
		for (auto object: InsIntegrable) {
			object->WriteInsHeader(FInsOutput);
		}
	}
...
void TOutputResults::OutputInstantaneousResults(TCalculoExtern *EXTERN, bool ThereIsDLL, bool EngineBlock, double Theta,
	...
	if(FPlotIns) {
		...
		for (auto object: InsIntegrable) {
			object->WriteInsResults(FInsOutput);
		}
#ifdef WriteINS
