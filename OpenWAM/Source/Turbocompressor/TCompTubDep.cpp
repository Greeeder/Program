...
double TCompTubDep::CalGastoNuevo(double MasaAire) {
	...
	try {
		...
		if(FIsAcoustic) {
			...
			FRendimientoAd = FRendimiento;
			FRendimiento = FRendimiento * Kef;
		}

		if(FDeltaTiempo > 0 && FDelay > 0 && FRC_init) {
		...
		ret_val = -FDensidad2 * FVelocidad2 * FAreaSalComp * __cons::ARef * Sqrt(FTemperatura10 / Mapa->getTempRef()) *
				  Mapa->getPresionRef() / __units::BarToPa(FPresCorrec);
	...
	try {
		...
		if(!valido) {
			...
			if(fMasa0 <= 0) {
				...
				GastoNuevo = Mapa->getGastoRelComp1() / (Sqrt(FTempCorrec / Mapa->getTempRef()) * Mapa->getPresionRef() /
							 __units::BarToPa(FPresCorrec));
				...
				GastoNuevo = -FVelocidad2 * __cons::ARef * FAreaSalComp * FDensidad2 * (Sqrt(FTempCorrec / Mapa->getTempRef()) /
							 Mapa->getPresionRef() * __units::BarToPa(FPresCorrec));
...
void TCompTubDep::CalculaCompresor(double Theta) {
	...
	try {
		...
		if(FIsAcoustic) {
			FTempCorrec = FAcComp->T10();
			FPresCorrec = FAcComp->P10();
		} else {
			FTempCorrec = FTemperatura10;
			FPresCorrec = FPresion10;
		}
		if(temp >= P20Max) {
			// *FBeta = *FLanda;
			// work = 0.;
			...
			FGasto1 = 0;
			//printf("WARNING: Surge %lf\n", FGasto1);
		} else {

			FGastoCorregido = RegulaFalsi();

			// if (FGastoCorregido > 1e-10)
			// printf("%lf ", FGastoCorregido);
			RC_ant = FRelacionCompresion;
			RC_filt_ant = RC_filt;

			// FGasto1 = NewDampedSolution(FGastoCorregido * FPresCorrec * 1e5 / Mapa->getPresionRef()
			// / sqrt(FTempCorrec / Mapa->getTempRef()));

			FGasto1 = (FGastoCorregido * __units::BarToPa(FPresCorrec) / Mapa->getPresionRef() / Sqrt(
						   FTempCorrec / Mapa->getTempRef()));
		}
		...
		*FBeta = FASonidoSalida - FGamma3 * FVelocidad2;
		...
		FGastoCorregido = FGasto1 / (__units::BarToPa(FPresCorrec) / Mapa->getPresionRef() / Sqrt(
										 FTempCorrec / Mapa->getTempRef()));
		FGasto0 = FGasto1;
