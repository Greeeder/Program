...
//FILE *fich=fopen(FileSALIDA,"a");
		...
		if(FMedias.GraficaMedias) {
			if(FMedias.GraficaTrabajo) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4021) + PutLabel(907);
				medoutput << Label.c_str();
			}
			if(FMedias.GraficaRendimiento) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4011) + PutLabel(901);
				medoutput << Label.c_str();
			}
			if(FMedias.GraficaRelacionCompresion) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4026) + PutLabel(901);
				medoutput << Label.c_str();
			}
			if(FMedias.GraficaGasto) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4008) + PutLabel(904);
				medoutput << Label.c_str();
			}
			if(FMedias.GraficaGastoCorregido) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4024) + PutLabel(905);
				medoutput << Label.c_str();
			}
			if(FMedias.GraficaRegimenCorregido) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4025) + PutLabel(906);
				medoutput << Label.c_str();
			}
		}
//fclose(fich);
...
//FILE *fich=fopen(FileSALIDA,"a");
		...
		if(FInstant.GraficaInstantaneas) {
			if(FInstant.GraficaPotencia) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4009) + PutLabel(903);
			...
			if(FInstant.GraficaGastoCorregido) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4024) + PutLabel(904);
				insoutput << Label.c_str();
			}
			if(FInstant.GraficaRegimenCorregido) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4025) + PutLabel(918);
				insoutput << Label.c_str();
			}
			if(FInstant.GraficaGamma) {
				Label = "\t" + PutLabel(5008) + "/" + std::to_string(FNumeroCompresor) + "/" + PutLabel(4017) + PutLabel(901);
				insoutput << Label.c_str();
			}
		}
//fclose(fich);
...
void TCompresor::PreprocessMap(double TIT, double Dw) {

	//Mapa->CalculateAdiabaticEfficiency(FHTM, TIT);

	Mapa->CalculateAdiabaticEfficiency(FHTM, TIT);

	Mapa->ExtrapolateMap(Dw);
