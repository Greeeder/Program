...
// Rango e incremento de regimenes de giro.
		...
		if(FNumCurvasReg != ncurv)
			cout << "ERROR: The number of iso-speed lines provided to the compressor map is not correct" << endl;

// Rango e incremento de gastos masicos.

		FGastoMin = GetXMLRotationalSpeed(node_map, "MinimumMassFlow") * FMassMultiplier;
		FGastoMax = GetXMLRotationalSpeed(node_map, "MaximumMassFlow") * FMassMultiplier;
		FIncGasto = GetXMLRotationalSpeed(node_map, "MassFlowStep") * FMassMultiplier;
...
// Lectura del mapa
		...
		for(xml_node node_spline = GetNodeChild(node_map, "Cmp_SpeedLine"); node_spline;
			node_spline = node_spline.next_sibling("Cmp_SpeedLine")) {
			FRegimenCurva[i] = FRegMin + FIncReg * (double) i;
			FGastoRelComp1[i] = (GetXMLMassFlow(node_spline, "MassCR1", unitmass) - 1) * FCRMultiplier + 1;
			FGastoBombeo[i] = GetXMLMassFlow(node_spline, "MassSurge", unitmass) * FMassMultiplier;
			FRelCompBombeo[i] = (GetAttributeAsDouble(node_spline, "CompRatioSurge") - 1) * FCRMultiplier + 1;

			j = 0;
			for(xml_node node_crpoint = GetNodeChild(node_spline, "Spl_CRPoint"); node_crpoint;
				node_crpoint = node_crpoint.next_sibling("Spl_CRPoint")) {
				FRelComp[i][j] = (GetAttributeAsDouble(node_crpoint, "CompRatio") - 1) * FCRMultiplier + 1;
				j++;
			}
			FNumCurvasRen[i] = 0;

			j = 0;
			for(xml_node node_effpoint = GetNodeChild(node_spline, "Spl_EFFPoint"); node_effpoint;
				node_effpoint = node_effpoint.next_sibling("Spl_EFFPoint")) {
				FGastoRend[i][j] = GetXMLMassFlow(node_effpoint, "MassFlow", unitmass) * FMassMultiplier;
				FRend[i][j] = GetAttributeAsDouble(node_effpoint, "Efficiency") * FEffMultiplier;
				if(FRend[i][j] > 0.) {
					++FNumCurvasRen[i];
				}
				if(FGastoRend[i][j] > FGastoRelComp1[i]) {
...
double TMapaComp::EvaluaRCSplines(double Massflow) {
	try {
		...
		std::cout << "Tipo de error: " << N.what() << std::endl;
		err << "ERROR: EvaluaRCSpline en el compresor: " << FNumeroCompresor << " " << N.what();
